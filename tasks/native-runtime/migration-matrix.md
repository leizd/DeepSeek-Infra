...
| search prefetch (slices 1-2) + context taint (searchContext) | rust | py | `search`: the three payload predicates + the two `format_*` + `search_multiple` (parallel) | **slices 1-2 of 4 ported; not wired** | byte-identical to oracle (`84b6f0f6…`, 2461 lines); lib-test harness cannot start on this host — see `continuation.md` (KERNEL32 lacks the futex exports) | remaining: `search_if_needed` + **`context_taint.harden_search_context` (383-line security module)** + the `build_deepseek_request` injection — the consumer still does not exist in Rust, so slice 1 is inert by design; the firewall gets the reference's own tables, like the IP-block sets |
