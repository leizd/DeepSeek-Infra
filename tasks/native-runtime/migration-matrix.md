...
| tool_catalog (28 definitions + schema index) | rust | py | `tool_catalog`: `available_tool_definitions` + `tool_parameter_schemas` + `schema_for_tool` + `agent_tool_definitions` | **ported; not wired** | byte-identical asset + 18-key parity probe (`a2fa62de…`); 231 tests; generated from the oracle rather than transcribed | the `mcp__` / external-MCP arms are injected; `None` reproduces the oracle escape hatch |
