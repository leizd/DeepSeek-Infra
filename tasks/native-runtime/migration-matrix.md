...
| search prefetch (slices 1-4) + per-turn context (searchContext) | rust | py | `search`: the three payload predicates + the two `format_*` + `search_multiple` (parallel); `context_taint`: **the whole module** — guard, both tables, alternation, `scan_text`, the hardening trio, `classify_request_messages`, `build_taint_report`, `report_is_tainted`, `taint_status`; `dynamic_context`: the reader of `searchContext` + `append_context_to_latest_user` + the per-turn helpers | **slices 1-4 ported; `context_taint` complete; not wired** | byte-identical to oracle (`84b6f0f6…` 2461 lines; `49e390b4…` 176 keys; `e3a065e9…` 39 keys); lib-test harness **fixed** by a committed `rust/.cargo/config.toml` rustflag (242 tests green) — see `continuation.md` | remaining: **`build_deepseek_request` — measured and NOT a slice** (~123 lines of assembly over a ~1200-line unported closure: `model_router` 279, `budget_manager` 371 with a ledger, `context_manager` 137, the validation/normalisation set ~110, `chat_payload` 32); the OS local-timezone resolution (no time crate in the workspace, deliberately not faked); `search_if_needed` |
