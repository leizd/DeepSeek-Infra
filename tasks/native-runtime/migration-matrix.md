...
| search prefetch + context taint (searchContext) | — | py | `none equivalent` | **not started, scoped** | — | `search_if_needed` + `search_multiple` (parallel) + the two `format_*` + **`context_taint.harden_search_context` (383-line security module)** + the `build_deepseek_request` injection. Its consumer does not exist in Rust, so the formatters alone would be inert — see `continuation.md` for the measured breakdown and the recommended order |
