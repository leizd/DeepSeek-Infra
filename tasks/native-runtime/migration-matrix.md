...
| tool_dispatch_seam (layer 2a) | rust | py | `deepseek-policy::tool_dispatch`: envelope, `parse_tool_arguments`/`safe_limit`/`tool_call_name`/`is_parallel_safe_tool`, branch inventory + routing, parse→gate→route order | **ported; not wired (no production caller)** | byte-identical to oracle (`9d491ef3…`), 49 keys; `docs/GATEWAY_TOOL_DISPATCH.md` | wire into the round loop only after the branch bodies land |
| tool_branch: generate_chart | rust | py | `tool_dispatch::generate_chart` + `chart_markdown_table` | **ported; not wired** | in the same 49-key diff | none |
| tool_branch: data_transform | rust | py | `tool_transform`: `data_transform` + 4 operations + helpers (json-path, csv, stats) | **ported; not wired** | in the 80-key batch diff (`3f088f27…`) | none |
| tool_batch (layer 2b) | rust | py | `tool_batch`: `execute_tool_calls` batching + cancellation + `role:"tool"` message assembly | **ported; not wired** | in the 80-key diff; concurrency not reproduced (sequential group = same index-ordered list) | thread pool is a mechanism; the 15 unported branches still block wiring |
| tool_branches: search family | rust | py | `tool_search`: `web_search` + `compare_search_results` (callback injected via `ExecutorContext`) + `search_result_key` | **ported; not wired** | in the 104-key diff (`a6aa9b0e…`) | none |
...
| data_branch: reminders | rust | py | `reminders`: store + `create_reminder` + `list_reminders` + `delete_reminder` + `due_reminders` + `parse_due_at` | **ported; not wired** | byte-identical to oracle (`a636cd45…`), 74 keys; 171 tests; file bytes incl. key order | `due_reminders` ported but not yet in a compared corpus |
| data_branch: memory triple | rust | py | `memory`: store + `suggest_memory` + `recall_memory` + `forget_memory` + normalization/fingerprint/category/conflict helpers | **ported; not wired; one known gap** | byte-identical to oracle (`4261dd06…`), 92 keys; 206 tests; `docs/MEMORY_STORE.md` | **vector-search bonus not ported** (`local_rag`, 2,676 lines) — `recall_memory` ranking is verified only where the vector index contributes nothing; the port reproduces the oracle's own `except Exception` path |
| workspace_file_lock (shared) | rust | py | `file_lock`: exclusive OS lock, `LockFileEx` retry / `flock` split | **ported** (used by `memory` and `mutation_gate`) | in the same 206 tests, incl. cross-thread serialization | none |
| data_branch: projects read path (slice E1) | rust | py | `projects`: id validation, the `normalize_*` family, `read_project`, `public_project`, `list_projects` | **ported; not wired** | byte-identical to oracle (`787f519d…`), 76 keys; `docs/PROJECTS_STORE.md` | **E2 not started**: `load_cached_file` + the two branch wrappers (`list_project_files`, `read_file_chunk`) |
| data_branch: projects branches (slice E2) | rust | py | `projects` branches + `file_cache`: `list_project_files` + `read_file_chunk` + `load_cached_file` + `project_file_cache_dir` | **ported; not wired** | byte-identical to oracle (`5baaaba2…`), 29 keys; `docs/PROJECTS_STORE.md` | **data layer now complete**; one open item: the A1 gate test failed once under load and needs root-causing |
| workspace_mutation_gate (slice A1) | rust | py | `mutation_gate`: fence read/assert, exclusive OS lock, durable generation counter, `mutation_scope` | **ported; not wired (no production caller)** | byte-identical to oracle (`57e0ede2…`), 32 keys; 155 tests incl. a multi-thread serialisation case; `docs/WORKSPACE_MUTATION_GATE.md` | slice B (reminders) is next |
...
| tool_branches: 14 remaining | rust | py | `none equivalent` | **unimplemented** | `Branch::blocker()` names each package; a test asserts none is silent | `browser_*`, `python_eval` (needs a real sandbox), `search_files`, `fetch_url`, memory/reminder/project branches, mindmap/pptx/document |
| tool_policy_pure_core (layer 3a) | rust | py | `deepseek-policy::tool_policy`: guards, sanitizers, schema validation, metadata + capability tables | **ported; not wired (no production caller)** | byte-identical to oracle (`26c7723c…`), 158 keys; 30 unit tests; `docs/GATEWAY_TOOL_POLICY_PARITY.md` | wire into layer 2 execution |
| tool_policy_engine (layer 3b) | rust | py | `deepseek-policy::tool_policy`: `ToolPolicy.evaluate`, denial output, diagnostics, taint, `AuditSink` + JSONL writer, args hash, `tool_policy_status` + `ToolPolicySettings` + `ToolAuditPaths` | **ported; not wired (no production caller)** | byte-identical to oracle (`bae3a9e5…`), 257 keys; 82 unit tests; `docs/GATEWAY_TOOL_POLICY_PARITY.md` | wire into layer 2 execution; config env reader not ported |
...
1. TLS transport qualification (this slice).
