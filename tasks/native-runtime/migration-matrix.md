...
| search prefetch (slices 1-3) + context taint (searchContext) | rust | py | `search`: the three payload predicates + the two `format_*` + `search_multiple` (parallel); `context_taint`: the guard, both pattern tables, the sensitive-tool alternation, `scan_text`, and `harden_search_context` / `file_context_guard_line` / `escalation_enabled` | **slices 1-3 of 4 ported; not wired** | byte-identical to oracle (`84b6f0f6…`, 2461 lines; `adff8e27…`, 87 keys); lib-test harness cannot start on this host — see `continuation.md` (KERNEL32 lacks the futex exports) | remaining: `search_if_needed` + the taint diagnostics half (`classify_request_messages`, `build_taint_report`, `report_is_tainted`, `_risk_level`, `taint_status`) + the `build_deepseek_request` injection — the consumer still does not exist in Rust, so these slices are inert by design; both taint tables are read off the reference, like the IP-block sets |
