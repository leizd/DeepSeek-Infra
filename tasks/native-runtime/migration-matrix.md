...
| `/api/chat` (NDJSON), search | `routes/chat.py` | rust or go `/api` | **wired on the native edge** — the ordinary streaming turn, including the tool-round loop, memory suggestions and the diagnostics block | `tests/chat_ndjson_route.rs` (7 real-HTTP cases through `create_production_app` against a scripted SSE upstream: the oracle's event sequence and order, the accumulated `done` **and its diagnostics**, the `length` note, agent mode and forced search refused with **zero** upstream calls, the no-user-turn 400, an upstream failure as an HTTP error rather than a 200 stream, and the auth boundary); `chat_stream_events_parity_probe.py` **PASS** over 21 events + 11 usage merges + 8 tool-call sequences + 4 tool-diagnostic cases + 17 usage-diagnostic cases (the `round(x, 1)` tie corpus) + 5 search-round counts + 6 search-diagnostic cases; `docs/NATIVE_PUBLIC_ROUTES.md` | **refused, not degraded**: `agentMode`, the model-router cascade and a forced `searchMode` return `501 NATIVE_CHAT_BRANCH_NOT_READY` before any upstream call (their producers are unported); search prefetch, edge inference and the gateway-attempt/semantic-cache/cost/trace diagnostics blocks are outstanding |
...
| `/api/download` | `routes/downloads.py` | rust edge | **wired** — `generated_files::resolve_generated_file` + `download_descriptor`; the id rule and the per-type MIME/name mapping stay in the policy crate | `tests/download_route.rs` (6 real-HTTP cases through `create_production_app`: the bytes on the wire are the bytes on disk for all five registered types, the `inline` rule for SVG — the web layer's `1/true/yes/on` parse — a traversal attempt leaking nothing, the unknown-id 404 envelope, and the auth boundary); `generated_files` unit test pins the oracle's six MIME/name pairs | the file/upload and page-render family (`/api/file-page-*`) is still the proxy's 503 |
| `/api/file-source` | `routes/files.py` | rust edge | **wired** — `file_routes::cached_file_source` + `content_disposition_header` + `original_file_media_type`; the id rule, the project-scoped directory and the media-type ladder stay in the policy crate | `tests/file_source_route.rs` (6 real-HTTP cases through `create_production_app`: the **original bytes** rather than the index JSON, the `download` string parse including `download=false` being inline, the media-type ladder, a missing source as **410** and a bad id as **400**, a project-scoped read from `.projects/{id}/files`, and the auth boundary); `file_routes_parity_probe.py` **PASS** over **201 cases** | `/api/file-page-image`, `/api/file-page-layout` and `/api/file-page-search` need a PDF/image renderer, which is not ported; `/api/file-text` (multipart upload + extraction) and `/api/project-files` are outstanding |
| `/api/file-reader`, `/api/file-chunk` | `web/server.py` | rust edge | **wired** — `file_routes::file_reader_window` + `file_chunk` + `reader_positive_int`; the 1-based display indices, the 12-chunk cap and the two error envelopes stay in the policy crate | `tests/file_reader_route.rs` (6 real-HTTP cases through `create_production_app`: the default 1/6 window, a later window's `hasPrevious`/`hasNext`, the falsy-value defaults with a per-field `400`, the 12-chunk cap, the empty-list shape, the 1-based single chunk with its `404`/`400`, an unknown id's `410`, and the auth boundary); `file_routes_parity_probe.py` **PASS** over **201 cases**, including 56 window shapes and 42 chunk lookups against the imported oracle | `/api/file-page-text` needs the page-text helpers (`normalize_extracted_text`, `page_texts_for_cache`), which are not ported |
...
6. All 18 chat-tool branches run; native `/mcp` `/a2a` (including SSE/resubscribe) `/api/tool-policy` `/api/budget` `/api/rag/status` `/api/gateway/status` `/api/taint` `/api/title` `/api/download` `/api/file-source` `/api/file-reader` `/api/file-chunk` `/api/chat` (ordinary streaming turns + tool rounds; agent mode, cascade and forced search refused); Go `/api/config` subset. Remaining: the skills registry/runner family, the rest of the file family (`/api/file-page-*` needs a renderer, `/api/file-page-text` needs the page-text helpers; `/api/file-text` needs multipart upload + extraction, `/api/project-files`), the whole `/api/workspace/*` backup/DR surface, the remaining diagnostics status blocks (`/api/mcp`, `/api/scheduler`, `/api/semantic-cache/status`, `/api/edge/status`, `/api/rust/status` — each needs its Python status function ported first), search prefetch and edge inference for `/api/chat`, A2A Python-task migration/retention/peer clients/telemetry/full parity, launchers.
