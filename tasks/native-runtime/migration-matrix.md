...
| search prefetch (slice 1) + context taint (searchContext) | rust | py | `search`: `search_mode` + `forced_search_mode` + `search_tool_enabled` + `format_search_context` + `format_search_failure_context` | **slice 1 of 4 ported; not wired** | byte-identical to oracle (`0f387780…`), 157 keys; 237 tests | remaining: `search_if_needed` + `search_multiple` (parallel) + **`context_taint.harden_search_context` (383-line security module)** + the `build_deepseek_request` injection — the consumer still does not exist in Rust, so slice 1 is inert by design; the firewall gets the reference's own tables, like the IP-block sets |
