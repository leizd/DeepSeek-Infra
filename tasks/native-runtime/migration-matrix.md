...
| data_branch: memory triple | rust | py | `memory`: store + `suggest_memory` + `recall_memory` + `forget_memory` + normalization/fingerprint/category/conflict helpers + **the turn-state half** (`prepare_memory_state`, `apply_explicit_memory_command`, `format_memory_context`, `memory_scope_candidates`/`_label`, `upsert_memory`, `clear_memories`, `delete_memory_by_id`) | **ported; wired via the loop's `WorkspaceContext`**; the turn-state half is ported but unwired | byte-identical to oracle (`97187819…`), 194 keys; 390 tests; `docs/MEMORY_STORE.md` | **vector bonus is not bounded — measured**: `memory_vector_bonus_probe.py` shows 7/8 queries change order *and* set with the index live (`LOCAL_RAG_ENABLED` default true, hash embeddings are offline). The wiring must inject a provider or refuse; `None` is a visible divergence |
