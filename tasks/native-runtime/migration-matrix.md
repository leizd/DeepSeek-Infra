...
| data_layer_storage (memory/reminders/projects) | rust | py | `none equivalent` | **measured, not implemented** | `docs/DATA_LAYER_MEASUREMENT.md` — media, schemas, migration semantics, and the fence coupling | slice order A→E; **every write participates in the backup generation protocol**, so `mutation_gate` is a prerequisite |
| workspace_mutation_gate | rust | py | `mutation_gate` fence + `.workspace-mutation.lock` + generation counter | **unimplemented** | 251 lines, self-contained; only `core.config.ROOT` + `core.errors` outside | prerequisite for any data-layer write; must keep `_fsync_directory` best-effort |
| core_utils_scorer | rust | py | `query_tokens` + `score_chunk` + `utc_now_iso` + `latest_user_query` | **unimplemented** | pure; shared by the memory branches and `search_files` | one port unblocks two branches |
