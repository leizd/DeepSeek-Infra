...
| tool_branches: 15 remaining | rust | py | `none equivalent` | **unimplemented** | `Branch::blocker()` names each package; a test asserts none is silent | `browser_*`, `python_eval` (needs a real sandbox), `search_files`, `fetch_url`, `web_search`, `compare_search_results`, memory/reminder/project branches, mindmap/pptx/document |
