...
| search prefetch (slices 1-4) + per-turn context (searchContext) | rust | py | `search`: the three payload predicates + the two `format_*` + `search_multiple` (parallel); `context_taint`: the guard, both pattern tables, the sensitive-tool alternation, `scan_text`, the hardening trio; `dynamic_context`: the reader of `searchContext` (`build_dynamic_turn_context`) + `append_context_to_latest_user` + the per-turn helpers and prompt constants | **slices 1-4 of 4 ported; not wired** | byte-identical to oracle (`84b6f0f6…` 2461 lines; `adff8e27…` 87 keys; `e3a065e9…` 39 keys); lib-test harness cannot start on this host — see `continuation.md` | remaining: `build_deepseek_request` — the body assembly that would actually consume the dynamic context, which does not exist in Rust (the gateway forwards the prepared raw body) — the OS local-timezone resolution (deliberately not faked: no time crate in the workspace), `search_if_needed`, and the taint diagnostics half. The taint tables, the slides prompt text and the clock are all taken from the reference rather than rebuilt |
