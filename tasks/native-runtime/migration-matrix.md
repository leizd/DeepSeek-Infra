...
| tool_branches: 17 remaining | rust | py | `none equivalent` | **unimplemented** | `Branch::blocker()` names each package; a test asserts none is silent | `browser_*`, `python_eval` (needs a real sandbox), `search_files`, `fetch_url`, `web_search`, `compare_search_results`, memory/reminder/project branches, `data_transform`, mindmap/pptx/document |
| tool_execute_batch (layer 2b) | rust | py | `execute_tool_calls` parallel batch + cancellation | **unimplemented** | none | serial/parallel split via `SERIAL_TOOL_NAMES`, cancellation |
