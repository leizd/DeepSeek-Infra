...
| web-search provider | rust | py | `none equivalent` | **not started, measured** | — | needs a real Tavily integration: `search_single_round` + key + per-request cache + citation counter + turn limit + `search_budget`. `ExecutorContext.web_search` is already the seam |
