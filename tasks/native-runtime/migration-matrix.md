...
| openai_facade_translation | rust | py | `openai_facade::openai_to_internal_payload` — the six forwarded fields (`model`/`messages`/`stream`/`thinkingEnabled`/`localBaseUrl`/`temperature`), the seven dropped (`tools`, `tool_choice`, `max_tokens`, `top_p`, `reasoning_effort`, `thinking`, everything else), and the two refusals (`invalid_payload`, 400) | **ported and probe-verified; not yet invoked by the route** | byte-identical to the real `openai_api.py:29` function (`openai_facade_parity_probe`, **56 keys, 12 204 chars**); 7 unit tests | **the native route still builds its body from the OpenAI request directly** (`request_preparation::prepare_chat_request`), which **forwards six fields the oracle drops** and **omits `thinkingEnabled`/`localBaseUrl` that the oracle sets** — a visible divergence on a public route (§五.11). Wiring it needs `AssemblyEnv::from_env` (settings + ledger + expander + clock), `request_base_url`, and the two refusals |
