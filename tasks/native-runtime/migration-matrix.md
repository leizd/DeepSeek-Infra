...
| search prefetch (slices 1-4) + per-turn context + request-shaping leaves | rust | py | `search`: the three payload predicates + the two `format_*` + `search_multiple` (parallel); `context_taint`: **the whole module** — guard, both tables, alternation, `scan_text`, the hardening trio, `classify_request_messages`, `build_taint_report`, `report_is_tainted`, `taint_status`; `dynamic_context`: the reader of `searchContext` + `append_context_to_latest_user` + the per-turn helpers; `request_shaping`: the parallel hint, `tools_for_payload`, `forced_artifact_tool_name`, `normalize_reasoning_effort`, `has_image_content`, `count_payload_attachments`, plus `empty_memory_state` / `memory_scope_from_payload`; `context_engine`: the token half — heuristics, estimators, breakdown, window lookup, budget plan, `token_trim` | **slices 1-4 ported; `context_taint` complete; not wired** | byte-identical to oracle (`84b6f0f6…` 2461 lines; `49e390b4…` 176 keys; `e3a065e9…` 39 keys); lib-test harness **fixed** by a committed `rust/.cargo/config.toml` rustflag (307 tests green) — see `continuation.md` | remaining: **`build_deepseek_request` — a milestone, not a slice**; its pure leaves are ported, and what is left is `context_engine`'s identity half (`base_context_id` / `build_context_diff` / `build_engine_diagnostics` — **blocked on a SHA-1 decision**: the crate depends on `sha2`, not `sha1`), then `context_manager` 137, `model_router` 279, `budget_manager` 371 (ledger-backed, so last), the validation/normalisation set ~110, and the assembly with the diagnostics serializer that owns key order; plus the OS local-timezone resolution (no time crate in the workspace) and `search_if_needed` |
