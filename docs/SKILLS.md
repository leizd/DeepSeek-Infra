...
适用版本：v2.7.0。
Applicable version: v2.7.0.

DeepSeek Infra v2.7.0 对 Skill 的定义如下：
...
v2.7.0 在主 Web UI 中新增了一个本地 Skill Workbench：
...
v2.7.0 在 Skill Workbench 中新增了自定义 Skill Builder，让用户无需手动编写 JSON 即可创作 Skill：
...
v2.7.0 引入了本地 Skill Pack，使一组 Skill 可以一起导入、导出、安装和绑定到项目。Skill Pack 是一个 `.skillpack.json` manifest：
...
v2.7.0 新增了一个本地 Skill 质量闭环。Workbench 的 `Eval` 标签页运行离线 Skill / Pack eval，显示通过/失败状态、平均分、用例数量、失败用例、最近运行元数据，并可导出 JSON / Markdown 摘要。Eval Case Builder 可创建基于规则的本地用例，无需手动编辑 JSONL。
...
```bash
python evals/runners/run_skill_eval.py --strict --out evals/reports/skills-v2.7.0.json
python evals/runners/run_skill_eval.py --scope skill --skill-id skill_study_tutor --out evals/reports/skills-v2.7.0.json
python evals/runners/run_skill_eval.py --scope pack --pack-id pack_study --out evals/reports/skills-v2.7.0.json
python evals/runners/run_skill_eval.py --baseline evals/reports/skills-v2.6.4.json --out evals/reports/skills-v2.7.0.json
```
...
v2.7.0 新增了 Skill run 的本地运行历史和使用统计。Runner 将已完成和失败的运行记录在 `.skills/runs/runs.jsonl` 中，仅包含元数据：`skillRunId`、`skillId`、`skillVersion`、`packId`、`projectId`、状态、时间戳、延迟、离线/模型标志、输入/输出摘要、artifact 和 saved-item 数量、`traceId` 以及诊断字段。
...
v2.7.0 新增了 Skill 和 Skill Pack 的本地安全审查和签名预备元数据。安全审查仍以本地优先：不存在远程 marketplace、远程签名服务器或基于账户的审批流程。
...
v2.7.0 新增本地 Skill Catalog / Marketplace-lite。Catalog 只索引本机已有的 Skill 和 Pack，不联网下载第三方内容，也不做远程签名服务器或用户账号体系。
...
发布 evidence 文件为 `docs/evidence/skills-v2.7.0.json`。
Skill Workbench UI evidence 文件为 `docs/evidence/skills-ui-v2.7.0.json`。
自定义 Skill Builder evidence 文件为 `docs/evidence/skill-builder-v2.7.0.json`。
Skill Packs evidence 文件为 `docs/evidence/skill-packs-v2.7.0.json`。
Skill Eval Dashboard evidence 文件为 `docs/evidence/skill-eval-dashboard-v2.7.0.json`。
Skill Versioning evidence 文件为 `docs/evidence/skill-versioning-v2.7.0.json`。
Skill Analytics evidence 文件为 `docs/evidence/skill-analytics-v2.7.0.json`。
Skill Security evidence 文件为 `docs/evidence/skill-security-v2.7.0.json`。
Skill Catalog evidence 文件为 `docs/evidence/skill-catalog-v2.7.0.json`。
Skill eval 报告为 `evals/reports/skills-v2.7.0.json`。
Skill eval report is `evals/reports/skills-v2.7.0.json`.
