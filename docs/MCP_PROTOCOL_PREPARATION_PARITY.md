...
python scripts/check_mcp_protocol_parity.py \
  ...
  --strict \
  --report artifacts/mcp-protocol-parity-report.json
```
...
The `mcp-protocol-parity` CI job uploads `artifacts/mcp-protocol-parity-report.json` even when the gate fails.
