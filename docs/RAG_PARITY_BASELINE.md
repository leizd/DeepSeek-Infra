...
python scripts/check_rag_parity.py \
  ...
  --strict \
  --report docs/evidence/rag-parity-v4.0.2.json
```
...
The independent `ci / rag-parity` job builds and starts only the Rust sidecar image, runs the strict 38-case corpus, uploads `docs/evidence/rag-parity-v4.0.2.json`, prints sidecar logs on failure, and always removes the container.
