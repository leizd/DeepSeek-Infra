...
DeepSeek Infra v2.7.0 makes media a first-class workspace object. A media item can belong to a project, keep source metadata, move through a processing lifecycle, emit citable segments, enter Local RAG, and travel with Project Export.
...
The JSON path supports webpage snapshots and transcript imports without requiring heavy local ASR/video dependencies. Audio/video built-ins are intentionally MVP in v2.7.0: metadata plus transcript/frame-caption imports.
...
v2.7.0 adds built-in Skills that accept `mediaIds`:
