...
这里验证的是默认 Python Runtime 作为 MCP client 消费外部 server 的“出方向桥接”。无状态 MCP 是独立的“入方向专用执行平面”，不消费 `MCP_CLIENT_SERVERS`；v4.4.2 为其增加逻辑备份 Contributor，但不改变这条协议边界。可靠性与可移植性合同见 [../STATELESS_MCP.md](../STATELESS_MCP.md)。
