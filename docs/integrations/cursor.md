...
本页配置和既有 GUI 证据针对默认 Python MCP Tool Hub（17 工具 + resources/prompts）。v4.4.1 的无状态五工具执行平面使用独立 `:8010/mcp`、`MCP_AUTH_TOKEN` 和 Redis；它尚未被本页的 Cursor GUI 证据覆盖，部署见 [../STATELESS_MCP.md](../STATELESS_MCP.md)。
