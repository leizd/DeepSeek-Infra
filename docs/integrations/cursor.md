...
本页配置和既有 GUI 证据针对默认 Python MCP Tool Hub（17 工具 + resources/prompts）。无状态五工具执行平面使用独立 `:8010/mcp`、`MCP_AUTH_TOKEN` 和 Redis；v4.4.2 增加逻辑备份但没有扩展本页的 Cursor GUI 证据，部署见 [../STATELESS_MCP.md](../STATELESS_MCP.md)。
