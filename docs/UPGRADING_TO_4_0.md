...
This guide covers the `4.0.0-rc.2` Python-first hybrid candidate. The upgrade is intentionally additive: no mandatory database migration, user-directory deletion, or Rust sidecar deployment is required.

## 3.10.0 to 4.0.0-rc.2

1. Back up the runtime data directory as you would for any prerelease install.
2. Install rc.2 and start the normal Python service. Do not add the optional Rust Compose file unless you already operate the sidecar.
...
The required `embedding TEXT` column and JSON values are still present, so 3.10.0 can read cache rows created by rc.2. Older SQLite readers ignore the added nullable BLOB columns. Do not run a destructive schema downgrade. With Rust flags off, operation returns to Python-only.
...
The versioned result is recorded in `docs/evidence/upgrade-rollback-v4.0.0-rc.2.json`.
