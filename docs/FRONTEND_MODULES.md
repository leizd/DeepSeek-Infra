...
v4.0.1 完成第二轮高风险边界拆分：`theme_boot.js` 在首屏以同源外部脚本初始化主题；`credential_store.js` 管理仅限当前标签页会话的密钥并清除旧 `localStorage` 明文；`upload_controller.js` 封装超时、取消与单次 settle；`workspace_tabs.js` 封装 ARIA tab 和键盘导航；`skill_builder.js` 承载 Skill Builder 纯数据逻辑。`static/sw.js` 使用版本化 `CORE_SHELL` / `OPTIONAL_SHELL`，统一移除缓存键查询参数，并对静态资源执行 stale-while-revalidate。`chat.js` 和 `skills.js` 继续负责 DOM 组装，但不再拥有这些职责的底层实现。
...
| `static/modules/credential_store.js` | `createCredentialSession` | API Key / Tavily Key 的 session-scoped 读取、更新、遗留明文迁移与 `localStorage` 清理。 |
| `static/modules/upload_controller.js` | `createUploadTask` | XHR 上传生命周期、240 秒默认超时、AbortError、取消句柄、认证头与响应规范化。 |
| `static/modules/workspace_tabs.js` | `initWorkspaceTabs` | Workspace tab 激活、ARIA 状态、焦点与方向键 / Home / End 导航。 |
| `static/modules/skill_builder.js` | `defaultBuilderSkill`、`buildInputSchemaFromFields`、`skillSchemaFields`、`sampleSkillForBuilder` | Skill Builder 的默认值、input schema 双向转换和示例数据；不依赖 DOM。 |
...
- `static/sw.js` 的 `CORE_SHELL` 必须列出启动必需模块；`OPTIONAL_SHELL` 只放允许独立失败的兼容和次级资源，否则离线/PWA 缓存会漏文件或安装失败。
