...
4.0.8 完成 Legacy Frontend Retirement；4.0.9 进一步将最后一个独立原生界面 Trace Viewer 迁入 React。4.1.0 将 Workspace Provider 下沉到聊天路由，并按需加载 Trace 路由与 Diagnostics 共享详情。4.2.2 为共享 Trace 加载边界增加原地重试和陈旧响应保护；4.2.3 为 Workspace Mutation 增加统一 rejection 收口、MutationCache 并发状态、实体锁与读取取消；4.2.4 进一步划分精确 Mutation 所有权、保留 recover 期间的活跃任务，并协调实体生命周期与 Memory 独占写入；4.2.5 将规范化 intent 与生命周期 meta 写入 MutationCache，使冲突可见且活跃状态能够跨 Workspace 重挂载恢复；4.2.6 进一步保护异步完成时的最新选择、草稿与记忆建议，并按实体归并错误和自动清理已解除的协调冲突。`/` 与 `/trace/:traceId` 只返回 `frontend/` 的 React + TypeScript + Vite 构建，`/ui/` 作为兼容别名返回同一构建。生成产物位于 gitignored `static/ui/`，不得手工修改。
