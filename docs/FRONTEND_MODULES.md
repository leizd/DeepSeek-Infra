...
4.0.8 完成 Legacy Frontend Retirement；4.0.9 将最后一个独立原生界面 Trace Viewer 迁入 React。4.1.0 将 Workspace Provider 下沉到聊天路由，并按需加载 Trace 路由与 Diagnostics 共享详情。4.2.2 为共享 Trace 加载边界增加原地重试和陈旧响应保护；4.2.3 为 Workspace Mutation 增加统一 rejection 收口、MutationCache 并发状态、实体锁与读取取消；4.2.4 划分精确 Mutation 所有权并协调实体生命周期与 Memory 独占写入；4.2.5 把规范化 intent 与生命周期 meta 写入 MutationCache；4.2.6 保护异步完成时的最新选择、草稿与记忆建议，并按实体归并错误；4.2.7 为每次生命周期加入稳定 `lifecycleId`，让冲突指向精确 blocker，并协调同一项目的删除与绑定保存。4.2.8 只闭环 exact-merge Evidence 装配与发布包验证，不改变前端状态架构、依赖、懒加载、持久化或 bundle budget。`/` 与 `/trace/:traceId` 只返回 `frontend/` 的 React + TypeScript + Vite 构建，`/ui/` 作为兼容别名返回同一构建。生成产物位于 gitignored `static/ui/`，不得手工修改。
...
| Workspace | `frontend/src/features/projects/`, `frontend/src/features/skills/`, `frontend/src/features/memory/`, `frontend/src/shared/runUiAction.ts`, `frontend/src/shared/useActionLocks.ts` | Projects, Skill management/binding and memory workflows; contained UI dispatch and keyed duplicate suppression |
