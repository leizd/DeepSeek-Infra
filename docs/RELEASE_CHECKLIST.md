...
4. Run `python scripts/preflight_release.py --version 4.4.5 --ga`.
5. Verify Edge Router stabilization evidence with `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v4.4.5.json`.
6. Verify Context Taint evidence with `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v4.4.5.json`.
...
11. Build both images with `docker build -t deepseek-infra:4.4.5 .` and `docker build -f stateless-mcp/Dockerfile -t deepseek-stateless-mcp:4.4.5 .`; validate both Compose files.
...
13. Run `cargo test --locked --manifest-path rust/Cargo.toml -p backup-crypto`; confirm passphrase/X25519 round trips and tamper rejection. A missing native linker is a local environment failure, not a PASS.
...
17. Generate the release zip, manifest and SHA-256 checksum with `python scripts/release.py --clean-workspace --version 4.4.5`; confirm the platform `backup-crypto` helper is bundled.
