...
4. Run `python scripts/preflight_release.py --version 4.4.11 --ga`.
5. Verify Edge Router stabilization evidence with `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v4.4.11.json`.
6. Verify Context Taint evidence with `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v4.4.11.json`.
...
11. Build both images with `docker build -t deepseek-infra:4.4.11 .` and `docker build -f stateless-mcp/Dockerfile -t deepseek-stateless-mcp:4.4.11 .`; validate both Compose files.
...
17. Run `pytest tests/test_backup_4411_contracts.py tests/test_backup_4410_contracts.py tests/test_backup_448_contracts.py` and confirm Effective Chunk Ref inheritance, transactional Map/Ref commit, Immediate Parent cross-file/whole-file reuse, v2/v3 compatibility, v4 immutable-parent restore, Bloom exact-lookup boundaries, batch fallback telemetry and strict Multipart convergence.
18. Run `cargo fmt --all -- --check`, `cargo clippy --locked --workspace --all-targets -- -D warnings` and `cargo test --locked --workspace`; confirm `deepseek-backup scan-batch` parity in a Rust-capable CI environment. A missing local MSVC linker is not a PASS.
19. Generate the release zip, manifest and SHA-256 checksum with `python scripts/release.py --clean-workspace --version 4.4.11`; confirm both platform helpers (`backup-crypto` and `deepseek-backup`) are bundled.
