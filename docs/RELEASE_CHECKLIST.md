...
4. Run `python scripts/preflight_release.py --version 4.4.10 --ga`.
5. Verify Edge Router stabilization evidence with `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v4.4.10.json`.
6. Verify Context Taint evidence with `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v4.4.10.json`.
...
11. Build both images with `docker build -t deepseek-infra:4.4.10 .` and `docker build -f stateless-mcp/Dockerfile -t deepseek-stateless-mcp:4.4.10 .`; validate both Compose files.
...
17. Run `pytest tests/test_backup_4410_contracts.py` and confirm public materialization, bounded restore reads, CDC protocol upgrade, Python/Rust parity, frozen adaptive plans and resumable multipart fencing.
18. Generate the release zip, manifest and SHA-256 checksum with `python scripts/release.py --clean-workspace --version 4.4.10`; confirm both platform helpers (`backup-crypto` and `deepseek-backup`) are bundled.
