...
> **Current decision: NOT READY FOR 4.0.0-rc.1.** ADR-0040 resolves all five runtime architecture blockers with an approved Python-first hybrid design. Conservative measured Python coverage is 90.52% across two 3.4.0 full-suite runs (90.57%, 90.52%) against the explicit 95.00% RC target and is the only remaining blocker.
...
| Current Python coverage gate | Python Runtime | PASS: 90% | `pyproject.toml` sets `fail_under = 90`; statement and branch coverage are measured together, with no separate branch threshold in 3.4.0. |
...
ADR-0040 is an approved architecture contract, not a runtime-default change. The 3.4.0 vector-ranking delegate does not modify `.env.example`, default Compose, the decision file, or runtime flag defaults.
