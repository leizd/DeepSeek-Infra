...
4.0.8 完成前端迁移：`/` 只加载 `frontend/` 经 React/TypeScript/Vite 构建出的 `static/ui/`，`/ui/` 是同一 React 应用的兼容别名。聊天、附件、Agent/activity、Projects、Skills、Memory、设置、Speech、Diagnostics 与 PWA 均由 React 拥有；历史原生入口、回滚环境变量和 Legacy 路由已退役。服务启动、Android、PyInstaller、Docker、release ZIP、release smoke 与 preflight 都把 `static/ui/index.html` 作为硬门禁，缺少构建产物时直接给出 `scripts/build_frontend.py` 的修复提示。
