...
3.9.0 adds only the compact binary route beside the compatible JSON vector endpoint; it adds no delegate. JSON remains the transport default and invalid configuration fails closed to JSON. Python reaches both through the same bounded process-local persistent connection pool, computes the complete authoritative ranking, and records preparation/serialization/transport/Rust-processing/validation/total timing separately. Binary failures return directly to Python without a second JSON sidecar request. The same sidecar listener exposes `GET /metrics` with fixed low-cardinality component/outcome/reason/encoding labels; neither metrics nor logs contain request payloads, vectors, credentials, paths, URLs, document IDs, tool arguments, or user metadata.
