...
前端迁移采用双入口隔离：默认 `/` 继续使用 `static/` 中的原生 ES modules，`/ui/` 只加载 `frontend/` 经 React/TypeScript/Vite 构建出的 `static/ui/`，两者不共享 DOM 所有权。4.0.3 的 `/ui/` 已串起普通聊天请求、NDJSON 流、Markdown 消息、停止生成、标题和本地会话历史；状态按 Chat/Settings/Overlay Context 分区，密钥仅驻留内存。附件、Agent/activity、Projects、Skills、Memory、高级设置、Speech、Diagnostics 与 PWA 仍由旧入口拥有，因此 `static/app.js` 和 `static/modules/chat.js` 保留。Docker 以 Node build stage 生成新入口，PyInstaller 与 release ZIP 在打包前执行同一确定性构建。
