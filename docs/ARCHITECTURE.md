...
- **Current release:** `4.0.2`，在稳定 `4.0.0` 上增加隔离的 React/TypeScript/Vite 迁移入口与纯流式状态域；默认 `/`、运行时所有权和冻结协议不变。
...
前端迁移采用双入口隔离：默认 `/` 继续使用 `static/` 中的原生 ES modules，`/ui/` 只加载 `frontend/` 经 React/TypeScript/Vite 构建出的 `static/ui/`，两者不共享 DOM 所有权。4.0.2 先把聊天/会话类型、纯 stream reducer、HTTP 错误和 NDJSON async iterator 放入 `frontend/src/domain` 与 `frontend/src/api`；复杂界面与服务端状态尚未迁移。旧入口的 `static/app.js` 仍只负责启动 `static/modules/chat.js`，其模块索引见 `docs/FRONTEND_MODULES.md`。Docker 以 Node build stage 生成新入口，PyInstaller 与 release ZIP 在打包前执行同一确定性构建。
