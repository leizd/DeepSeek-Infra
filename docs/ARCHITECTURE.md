...
- **Stable release:** `4.0.0`，在观察期结束后通过独立 promotion PR 从已验证的 `4.0.0-rc.2` 候选版本提升。
- **`v4.0.0-rc.1`** 已被 rc.2 supersede，只保留为历史架构预览。
- **Promotion boundary:** stable 只改变发布身份；Python-first 所有权、默认关闭的 Rust delegates 和冻结协议均不改变。
