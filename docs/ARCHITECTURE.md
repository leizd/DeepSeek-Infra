...
4.0.8 完成 Legacy Frontend Retirement；4.1.0 将最后一个独立原生界面 Trace Viewer 迁入 `frontend/src/features/trace/`，并用 React Router 提供 `/trace/:traceId`。Diagnostics 与独立 Trace 页共享摘要、span tree、waterfall、分类和错误组件。`/`、`/ui/` 与 Trace 深层路由都由同一 `static/ui/` 构建拥有；服务启动、Android、PyInstaller、Docker、release ZIP、release smoke 与 preflight 继续把 `static/ui/index.html` 作为硬门禁。
