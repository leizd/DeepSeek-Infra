...
3.8.0 does not add a route. Python reaches these endpoints through a bounded process-local persistent connection pool, still computes or validates the authoritative Python contract, and records preparation/serialization/transport/Rust-processing/validation/total timing separately. The same sidecar listener exposes `GET /metrics` with fixed low-cardinality component/outcome/reason labels; neither metrics nor logs contain request payloads, credentials, paths, URLs, document IDs, tool arguments, or user metadata.
