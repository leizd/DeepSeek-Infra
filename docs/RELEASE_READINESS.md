...
Release v4.4.5 — **Fenced Backup Commits & Replica Lineage**。本版把自动备份收成事务闭环：Run Lease Guard、目标 Writer Lease、不可变 Slot Commit、Catalog 投影 CAS、目标谱系检测、Retention 快照 CAS、Mirror Generation/Epoch/Sequence 围栏与前端单标签页上传选举。根 `VERSION` 仍是版本事实源；正式 PASS 只来自对应提交的 CI 与 exact-merge evidence。
## 1. Release Preflight — 版本一致性体检

发版前确认版本号在所有该出现的地方都同步，eval 报告是当前版本，且发布脚本仍排除本地缓存 / 日志 / 密钥：

```bash
python scripts/preflight_release.py --version 4.4.5
```

检查项：

- README 版本徽章是 `4.4.5`。
- `CHANGELOG.md` 顶部有 `## [4.4.5]` 条目。
- `Dockerfile` 示例 tag 是 `deepseek-infra:4.4.5`。
- `docs/IMPLEMENTATION_STATUS.md` 与 `evals/README.md` 的「适用版本」是 `v4.4.5`。
...
- `evals/reports/latest.json`、`agent-latest.json`、`baseline-compare-latest.json` 与 `security-latest.json` 的 `version` 是 `4.4.5`，且包含统一 metadata。
- `docs/evidence/headless-mcp-bridge.json` 可解析、版本为 `4.4.5`，且关键 MCP bridge 步骤全为 PASS。
- `docs/evidence/a2a-external-peer.json` 可解析、版本为 `4.4.5`，且关键 A2A external peer checks 全为 PASS。
...
- `docs/evidence/edge-router-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 doctor / status shape / route-preview API / fake provider / routing policy / fallback / forced-local 409 checks 全 PASS。
...
- `docs/evidence/workspace-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 Project / Saved Items / Artifact / Export / secret redaction checks 全 PASS。
- `docs/evidence/media-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 image / PDF / webpage import、segments、media-to-RAG、citations、project export 与 secret redaction checks 全 PASS。
- `docs/evidence/browser-v4.4.5.json` must exist for Browser Control releases, report `status=PASS`, and prove session creation, page read, screenshot, link extraction, private-host blocking, confirmation gates, Media snapshots, RAG chunks and audit logging.
- `docs/evidence/automation-v4.4.5.json` must exist for Automation Runtime releases, report `status=PASS`, and prove create/manual/schedule/event/Skill/Browser read-only/export/safety/history/trace/artifact/template checks.
- `docs/evidence/skills-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 Skill API route / registry / runner / artifact / project binding checks 全 PASS。
- `docs/evidence/skills-ui-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 Skill Workbench entrypoint / schema form / project binding / result links / styles / JS syntax / CI syntax gate checks 全 PASS。
- `docs/evidence/skill-builder-v4.4.5.json`、`docs/evidence/skill-packs-v4.4.5.json`、`docs/evidence/skill-eval-dashboard-v4.4.5.json`、`docs/evidence/skill-versioning-v4.4.5.json`、`docs/evidence/skill-analytics-v4.4.5.json`、`docs/evidence/skill-security-v4.4.5.json`、`docs/evidence/skill-catalog-v4.4.5.json` 与 `evals/reports/skills-v4.4.5.json` 必须存在、版本匹配、`status=PASS`，且 Skill authoring / Pack / Eval / Versioning / Analytics / Security / Catalog checks 全 PASS。
- `docs/evidence/context-taint-v4.4.5.json` 必须存在、版本为 `4.4.5`、`status=PASS`，且 web / file / media injection、tool directive、tainted-turn escalation、risk diagnostics checks 全 PASS。
...
```text
dist/deepseek-infra-4.4.5.zip
dist/deepseek-infra-4.4.5.zip.sha256
dist/deepseek-infra-4.4.5.manifest.json
```
...
```json
{
  ...
  "artifact": "deepseek-infra-4.4.5.zip",
  "sha256": "...",
  "bytes": 1234567
}
```
...
- run: python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v4.4.5.json
- run: python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v4.4.5.json
- run: python scripts/smoke_media.py --offline --out docs/evidence/media-v4.4.5.json
- run: python scripts/smoke_browser.py --offline --out docs/evidence/browser-v4.4.5.json --version 4.4.5
- run: python scripts/smoke_automation.py --offline --out docs/evidence/automation-v4.4.5.json --version 4.4.5
- run: python scripts/smoke_skills.py --offline --out docs/evidence/skills-v4.4.5.json
- run: python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v4.4.5.json
- run: python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v4.4.5.json
- run: python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v4.4.5.json
- run: python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v4.4.5.json --report-out evals/reports/skills-v4.4.5.json
- run: python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v4.4.5.json
- run: python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v4.4.5.json
- run: python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v4.4.5.json
- run: python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v4.4.5.json
- run: python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v4.4.5.json
- run: python evals/runners/run_media_eval.py --strict --out evals/reports/media-v4.4.5.json
- run: python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v4.4.5.json --version 4.4.5
- run: python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v4.4.5.json --version 4.4.5
- run: python scripts/preflight_release.py --version 4.4.5
- run: python scripts/doctor.py --offline
- run: python scripts/release.py --clean-workspace --dry-run
```

CI 不强制安装真实第三方 A2A server、Ollama 或 GGUF 模型；第三方/实机 evidence 缺失时是 WARNING。Edge Router 的 v4.4.5 dry-run evidence 不依赖真模型，属于硬门禁。
...
`preflight_release.py` 自 v2.8.1 起增加 `edge_router_evidence` 硬检查。它读取 `docs/evidence/edge-router-v4.4.5.json`，确认 Edge Router 在无真实模型、无 optional backend 的离线环境里仍可验证：
...
```bash
python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v4.4.5.json
```
...
## 11. Workspace Core Evidence（v4.4.5）

`preflight_release.py` 自 v2.5.0 起增加 `workspace_core_evidence` 硬检查。它读取 `docs/evidence/workspace-v4.4.5.json`，确认 Workspace Core 已经用离线 smoke 跑通：
...
```bash
python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v4.4.5.json
```

## 12. Media Layer Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `media_layer_evidence` 硬检查。它读取 `docs/evidence/media-v4.4.5.json`，确认 Multimodal Media Layer 已经完成离线核心验收：
...
```bash
python scripts/smoke_media.py --offline --out docs/evidence/media-v4.4.5.json
python evals/runners/run_media_eval.py --strict --out evals/reports/media-v4.4.5.json
```

## 13. Skill System Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_system_evidence` 硬检查。它读取 `docs/evidence/skills-v4.4.5.json`，确认 Skill System 已经完成 Web API 接入与离线核心验收：
...
```bash
python scripts/smoke_skills.py --offline --out docs/evidence/skills-v4.4.5.json
```


## 13. Skill Workbench UI Evidence（v4.4.5）
`preflight_release.py` 自 v2.7.0 起增加 `skill_ui_evidence` 硬检查。它读取 `docs/evidence/skills-ui-v4.4.5.json`，确认 Skill Workbench 前端已经完成本地 UI 接入与离线验收：
...
```bash
python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v4.4.5.json
```

## 14. Skill Builder Evidence (v4.4.5)

`preflight_release.py` 自 v2.7.0 起增加 `skill_builder_evidence` 硬检查。它读取 `docs/evidence/skill-builder-v4.4.5.json` 并验证本地创作路径：
...
```bash
python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v4.4.5.json
```

## 15. Skill Packs Evidence (v4.4.5)

`preflight_release.py` 自 v2.7.0 起增加 `skill_packs_evidence` 硬检查。它读取 `docs/evidence/skill-packs-v4.4.5.json` 并验证本地 Skill Pack 路径：
...
```bash
python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v4.4.5.json
```

## 16. Skill Eval Dashboard Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_eval_dashboard_evidence` 硬检查。它读取 `docs/evidence/skill-eval-dashboard-v4.4.5.json` 和 `evals/reports/skills-v4.4.5.json`，然后验证本地 Skill 质量路径：
...
```bash
python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v4.4.5.json --report-out evals/reports/skills-v4.4.5.json
```

## 17. Skill Versioning Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_versioning_evidence` 硬检查。它读取 `docs/evidence/skill-versioning-v4.4.5.json`，然后验证本地 Skill / Pack 生命周期路径：
...
```bash
python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v4.4.5.json
```

## 18. Skill Analytics Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_analytics_evidence` 硬检查。它读取 `docs/evidence/skill-analytics-v4.4.5.json`，然后验证本地 Skill 使用回路：
...
```bash
python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v4.4.5.json
```

## 17. Evidence Index & Metadata（v2.3.4）


## 19. Skill Security Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_security_evidence` 硬检查。它读取 `docs/evidence/skill-security-v4.4.5.json`，然后验证本地 Skill 信任路径：
...
```bash
python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v4.4.5.json
```

## 20. Skill Catalog Evidence（v4.4.5）

`preflight_release.py` 自 v2.7.0 起增加 `skill_catalog_evidence` 硬检查。它读取 `docs/evidence/skill-catalog-v4.4.5.json`，然后验证本地 Skill Marketplace-lite 路径：
...
```bash
python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v4.4.5.json
```
...
python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v4.4.5.json
python scripts/smoke_media.py --offline --out docs/evidence/media-v4.4.5.json
python scripts/smoke_browser.py --offline --out docs/evidence/browser-v4.4.5.json --version 4.4.5
python scripts/smoke_automation.py --offline --out docs/evidence/automation-v4.4.5.json --version 4.4.5
python scripts/smoke_skills.py --offline --out docs/evidence/skills-v4.4.5.json
python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v4.4.5.json
python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v4.4.5.json
python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v4.4.5.json
...
python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v4.4.5.json
python evals/runners/run_media_eval.py --strict --out evals/reports/media-v4.4.5.json
python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v4.4.5.json --version 4.4.5
python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v4.4.5.json --version 4.4.5
...
## 19. Quality Gate Evidence（v4.4.5）
...
- Workspace Core：`docs/evidence/workspace-v4.4.5.json` `status=PASS`。
- Media Layer：`docs/evidence/media-v4.4.5.json` 和 `evals/reports/media-v4.4.5.json` `status=PASS`。
- Automation Runtime：`docs/evidence/automation-v4.4.5.json` 和 `evals/reports/automation-v4.4.5.json` `status=PASS`。
- Skill System：`docs/evidence/skills-v4.4.5.json` `status=PASS`。
- Skill Workbench UI：`docs/evidence/skills-ui-v4.4.5.json` `status=PASS`。
- Skill Builder：`docs/evidence/skill-builder-v4.4.5.json` `status=PASS`。
- Skill Packs：`docs/evidence/skill-packs-v4.4.5.json` `status=PASS`。
- Skill Eval Dashboard：`docs/evidence/skill-eval-dashboard-v4.4.5.json` 和 `evals/reports/skills-v4.4.5.json` `status=PASS`。
- Skill Versioning：`docs/evidence/skill-versioning-v4.4.5.json` `status=PASS`。
- Skill Analytics：`docs/evidence/skill-analytics-v4.4.5.json` `status=PASS`。
- Skill Security：`docs/evidence/skill-security-v4.4.5.json` `status=PASS`。
- Skill Catalog：`docs/evidence/skill-catalog-v4.4.5.json` `status=PASS`。
...
## Stateless MCP reliability gate（v4.4.5）
...
只有对应提交的 GitHub checks 全绿时，才能声称无状态 MCP reliability gate 通过。Redis AOF 本身仍不进入 release ZIP 或默认 `/data` 卷；配置 external Contributor 后，4.4.5 只导出可移植的逻辑任务快照。
...
python scripts/release.py --clean-workspace --version 4.4.5
