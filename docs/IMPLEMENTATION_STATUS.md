...
> 4.3.6 partitions conversation persistence into per-conversation V3 shards (digest-verified head/snapshot, `<seq>.<tabId>` revisions with parentRevision chains) so editing one conversation serializes only that conversation with O(1) cleanup, arbitrates cross-tab commits inside an exclusive Web Lock with a lock-free sibling-detection fallback — re-reading the shared head in the critical section and preserving losing branches as recoverable conflict copies — commits bounded tombstones before deletion touches UI, sinks current-conversation selection into per-tab sessionStorage, throttles streaming saves to 1Hz with a trailing edge, degrades quota exhaustion deterministically (preview stripping, payload caps, compaction metadata) without touching user bodies, and writes per-tab recovery capsules on page exit that startup reconciles exactly once. The vendor runtime splits into a cacheable core chunk (entry 390KB→157KB) with bundle budgets unchanged. All 4.3.5 canonical-identity, diagnosable-flush, lossless-draft, journaled-checkpoint, honest-interruption and BFCache resync behavior, 4.3.4 activation-transaction/page-lifecycle persistence, 4.3.3 discovery/quiescent-reload, 4.3.2 identity/Lease behavior, 4.3.1 continuity and bundle budgets remain unchanged. The 4.2.8 exact-merge Evidence chain and frozen 4.0 runtime contract remain intact.
