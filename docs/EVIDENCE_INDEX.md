...
The `storage-control-plane-minio-e2e` exact-merge producer now owns a required
three-artifact set for 4.7.6:
...
The predictive proof is generated by a real three-MinIO Backup → Repair →
Rebalance → provider-capacity-probe → Forecast Registry → automatic backtest →
write-deny What-If execution. Its `predictive-planning-proof-v1` typed payload
binds the exact capacity observation set, Forecast Record/backtest calibration,
versioned price catalog, fresh-state bundle, candidate plan, and independently
measured pre/post state and storage inventories. Legacy What-If zero-write and
optimizer binding claims are promoted from scenario-test provenance to this
same typed runtime proof.

The report's `proofArtifacts` map binds both proof paths, scenarios, SHA-256,
and byte sizes. Producer staging and global Assembly reload the exact bytes and
repeat every semantic validator. Missing predictive proof, a self-reported zero
mutation counter, a tampered digest, or a report/proof mismatch fails closed.

Operators can validate the predictive artifact independently:
...
Local provider-backed success remains developer evidence only; formal PASS
requires the final merge SHA's CI artifact and successful Evidence Assembly.
Wire-format claims remain frozen:
...
The `storage-control-plane-minio-e2e` exact-merge producer owns both of these
artifacts for 4.7.4:

- `docs/evidence/storage-control-plane-minio-v4.7.4.json`
- `docs/evidence/storage-control-plane-autonomous-proof-v4.7.4.json`

The first is the report; the second is the exact `evidence-proof-v2` JSON that
made the autonomous scenario eligible to PASS. The report's `proofArtifact`
object binds the proof path, SHA-256, byte size, and
`real-three-minio-autonomous-remediation` scenario. Producer staging and final
Evidence Assembly independently reload those same bytes, repeat every semantic
validator, and fail if the proof is absent, changed, or belongs to another
scenario. A report-only PASS is therefore insufficient.

The autonomous proof is generated by
`tests/test_backup_472_real_three_minio_remediation_e2e.py` through
`scripts/run_storage_control_plane_minio_e2e.py`. It must include:

- actual Receipt v4 and Commit v4 bytes read back from the resolved MinIO
  endpoint/bucket/object keys, with provider size, ETag, and byte digests;
- a recomputed Receipt SHA-256 equal to `Commit.receiptDigest`, plus the same
  `backupId` and `objectSetDigest` in both objects;
- a live Repair crash/takeover trace with distinct OS PIDs, a strictly higher
  execution epoch, `RECONCILING` before mutation, and one underlying Repair job;
- independent two-process global, target, policy, and failure-domain admission
  races; and
- monotonic copy/domain floors calculated with running effects included.

Operators can revalidate a downloaded exact proof without trusting report
status:

```powershell
python scripts/validate_evidence_proof.py `
  --proof docs/evidence/storage-control-plane-autonomous-proof-v4.7.4.json `
  --scenario real-three-minio-autonomous-remediation
```

The command returns the exact byte count/SHA-256 and exits nonzero for any
schema, scenario, status, or semantic error. Locally generated real-MinIO proof
is developer evidence only; formal PASS still requires the final merge SHA's CI
artifact and successful Evidence Assembly.
...
- `docs/evidence/ga-v4.7.5.json`
- `docs/evidence/workspace-v4.7.5.json`
- `docs/evidence/edge-router-v4.7.5.json`
- `docs/evidence/media-v4.7.5.json`
- `docs/evidence/browser-v4.7.5.json`
- `docs/evidence/automation-v4.7.5.json`
- `docs/evidence/skills-v4.7.5.json`
- `docs/evidence/skills-ui-v4.7.5.json`
- `docs/evidence/skill-builder-v4.7.5.json`
- `docs/evidence/skill-packs-v4.7.5.json`
- `docs/evidence/skill-eval-dashboard-v4.7.5.json`
- `docs/evidence/skill-versioning-v4.7.5.json`
- `docs/evidence/skill-analytics-v4.7.5.json`
- `docs/evidence/skill-security-v4.7.5.json`
- `docs/evidence/skill-catalog-v4.7.5.json`
- `docs/evidence/context-taint-v4.7.5.json`
- `docs/evidence/semantic-cache-onnx-v4.7.5.json`
- `docs/evidence/upgrade-rollback-v4.7.5.json`
- `docs/evidence/protocol-contract-v4.7.5.json`
- `docs/evidence/frontend-bundle-v4.7.5.json`
- `docs/evidence/frontend-browser-v4.7.5.json`
- `evals/reports/latest.json`
- `evals/reports/agent-latest.json`
- `evals/reports/baseline-compare-latest.json`
- `evals/reports/security-latest.json`
- `evals/reports/skills-v4.7.5.json`
- `evals/reports/media-v4.7.5.json`
- `evals/reports/browser-v4.7.5.json`
- `evals/reports/automation-v4.7.5.json`
- `docs/evidence/rust-sidecar-image-v4.7.5.json`
- `docs/evidence/hybrid-runtime-e2e-v4.7.5.json`
- `docs/evidence/gateway-request-parity-v4.7.5.json`
- `docs/evidence/mcp-protocol-parity-v4.7.5.json`
- `docs/evidence/rag-parity-v4.7.5.json`
- `docs/evidence/rag-document-preparation-parity-v4.7.5.json`
- `docs/evidence/rag-vector-binary-parity-v4.7.5.json`
- `docs/evidence/rust-coverage-v4.7.5.json`
- `docs/evidence/rust-sidecar-performance-v4.7.5.json`
- `docs/evidence/packed-delta-s3-v4.7.5.json`
- `docs/evidence/object-set-s3-v4.7.5.json`
- `docs/evidence/recovery-faults-v4.7.5.json`
- `docs/evidence/replica-healing-s3-v4.7.5.json`
- `docs/evidence/storage-control-plane-minio-v4.7.5.json`
- `docs/evidence/storage-control-plane-autonomous-proof-v4.7.5.json`
