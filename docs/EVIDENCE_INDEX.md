...
## 4.4.13 candidate evidence contract

The following paths are the required CI output contract for 4.4.13. Files that depend on
...
- `docs/evidence/semantic-cache-onnx-v4.4.13.json`
- `docs/evidence/upgrade-rollback-v4.4.13.json`
- `docs/evidence/protocol-contract-v4.4.13.json`
- `docs/evidence/frontend-bundle-v4.4.13.json`
- `docs/evidence/frontend-browser-v4.4.13.json`
...
- `docs/evidence/rust-sidecar-image-v4.4.13.json`
- `docs/evidence/hybrid-runtime-e2e-v4.4.13.json`
- `docs/evidence/gateway-request-parity-v4.4.13.json`
- `docs/evidence/mcp-protocol-parity-v4.4.13.json`
- `docs/evidence/rag-parity-v4.4.13.json`
- `docs/evidence/rag-document-preparation-parity-v4.4.13.json`
- `docs/evidence/rag-vector-binary-parity-v4.4.13.json`
- `docs/evidence/rust-coverage-v4.4.13.json`
- `docs/evidence/rust-sidecar-performance-v4.4.13.json`
- `docs/evidence/packed-delta-s3-v4.4.13.json`

4.4.13 is the projected-recovery and production-remote-restore release. It freezes a remote restore into a Contributor/Project projection whose `selectionDigest` is durable across retries, separates a read-only Support dependency set from selected Outputs, verifies the full logical Merkle chain while materializing only selected payload bytes, reports whole-age-object download honestly, releases remote holds at terminal states, and drives a real HTTP MinIO production Backup → Age → S3 → Receipt → Restore → Federated-commit path with the real Rust Age helper. It retains the 4.4.12 packed-delta index/container contracts, 4.4.11 immediate-parent reuse, 4.4.10 bounded streaming/native parity, 4.4.9 true-delta verified chains, earlier fenced backup/restore and portability contracts, all 4.3.7 convergence contracts, and the frozen 4.0 protocol surface. The root `VERSION` file is canonical, and CI derives candidate/exact-merge evidence names and Docker tags from `RELEASE_VERSION`.
