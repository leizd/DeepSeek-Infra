...
The following paths are the required CI output contract for 4.4.11. Files that depend on
...
- `docs/evidence/semantic-cache-onnx-v4.4.11.json`
- `docs/evidence/upgrade-rollback-v4.4.11.json`
- `docs/evidence/protocol-contract-v4.4.11.json`
- `docs/evidence/frontend-bundle-v4.4.11.json`
- `docs/evidence/frontend-browser-v4.4.11.json`
...
- `docs/evidence/rust-sidecar-image-v4.4.11.json`
- `docs/evidence/hybrid-runtime-e2e-v4.4.11.json`
- `docs/evidence/gateway-request-parity-v4.4.11.json`
- `docs/evidence/mcp-protocol-parity-v4.4.11.json`
- `docs/evidence/rag-parity-v4.4.11.json`
- `docs/evidence/rag-document-preparation-parity-v4.4.11.json`
- `docs/evidence/rag-vector-binary-parity-v4.4.11.json`
- `docs/evidence/rust-coverage-v4.4.11.json`
- `docs/evidence/rust-sidecar-performance-v4.4.11.json`

4.4.11 is the effective-snapshot dedup-index and cross-file reuse release. It adds immutable Chunk Maps with inherited Snapshot references, immediate-parent exact cross-file and whole-file reuse, local-only Bloom acceleration, `incremental-v4` Parent Ranges, immutable-parent two-phase restore, Rust batch scanning, accurate fallback telemetry and exact S3 multipart convergence. It retains the 4.4.10 bounded streaming/native parity contracts, the 4.4.9 true-delta and verified-chain contracts, earlier fenced backup/restore and portability contracts, all 4.3.7 convergence contracts, and the frozen 4.0 protocol surface. The root `VERSION` file is canonical, and CI derives candidate/exact-merge evidence names and Docker tags from `RELEASE_VERSION`.
...
| Personal AI Runtime GA | `release-readiness` | `docs/evidence/ga-v4.4.11.json`; `scripts/smoke_ga.py` |
| Workspace Core | `release-readiness` | `docs/evidence/workspace-v4.4.11.json`; `scripts/smoke_workspace.py` |
| Edge Router stabilization | `release-readiness` | `docs/evidence/edge-router-v4.4.11.json`; `scripts/smoke_edge_router.py` |
| Media Layer | `release-readiness` / `eval` | `docs/evidence/media-v4.4.11.json`; `evals/reports/media-v4.4.11.json`; `scripts/smoke_media.py`; `run_media_eval.py` |
| Browser Control | `release-readiness` / `eval` | `docs/evidence/browser-v4.4.11.json`; `evals/reports/browser-v4.4.11.json`; `scripts/smoke_browser.py`; `run_browser_eval.py` |
| Automation Runtime | `release-readiness` / `eval` | `docs/evidence/automation-v4.4.11.json`; `evals/reports/automation-v4.4.11.json`; `scripts/smoke_automation.py`; `run_automation_eval.py` |
| Skill System | `release-readiness` | `docs/evidence/skills-v4.4.11.json`; `scripts/smoke_skills.py` |
| Skill Workbench UI | `release-readiness` | `docs/evidence/skills-ui-v4.4.11.json`; `scripts/smoke_skills_ui.py` |
| Skill Builder | `release-readiness` | `docs/evidence/skill-builder-v4.4.11.json`; `scripts/smoke_skill_builder.py` |
| Skill Packs | `release-readiness` | `docs/evidence/skill-packs-v4.4.11.json`; `scripts/smoke_skill_packs.py` |
| Skill Eval Dashboard | `release-readiness` / `eval` | `docs/evidence/skill-eval-dashboard-v4.4.11.json`; `evals/reports/skills-v4.4.11.json`; `scripts/smoke_skill_eval_dashboard.py` |
| Skill Versioning | `release-readiness` | `docs/evidence/skill-versioning-v4.4.11.json`; `scripts/smoke_skill_versioning.py` |
| Skill Analytics | `release-readiness` | `docs/evidence/skill-analytics-v4.4.11.json`; `scripts/smoke_skill_analytics.py` |
| Skill Security | `release-readiness` | `docs/evidence/skill-security-v4.4.11.json`; `scripts/smoke_skill_security.py` |
| Skill Catalog | `release-readiness` | `docs/evidence/skill-catalog-v4.4.11.json`; `scripts/smoke_skill_catalog.py` |
| Context Taint | `release-readiness` | `docs/evidence/context-taint-v4.4.11.json`; `scripts/smoke_context_taint.py` |
| Semantic Cache ONNX | `release-readiness` | `docs/evidence/semantic-cache-onnx-v4.4.11.json`; `benchmarks/bench_semantic_cache.py`; `docs/RUST_CANDIDATE_AUDIT_3_4.md` |
| Upgrade / rollback | `release-readiness` | `docs/evidence/upgrade-rollback-v4.4.11.json`; `scripts/generate_4_0_contract_evidence.py` |
| Protocol freeze | `release-readiness` | `docs/evidence/protocol-contract-v4.4.11.json`; `scripts/generate_4_0_contract_evidence.py` |
| Frontend bundle | `frontend` | `docs/evidence/frontend-bundle-v4.4.11.json`; `scripts/check_frontend_bundle.py`; Vite manifest |
| Frontend browser | `frontend-browser` | `docs/evidence/frontend-browser-v4.4.11.json`; `scripts/smoke_frontend_browser.py`; real Chromium |
...
| --- | --- | --- |
| Rust sidecar image | `rust-docker` | `docs/evidence/rust-sidecar-image-v4.4.11.json`; immutable digest from the exact merge job |
| Hybrid runtime E2E | `hybrid-runtime-e2e` | `docs/evidence/hybrid-runtime-e2e-v4.4.11.json`; `scripts/smoke_hybrid_runtime.py` |
| Gateway request preparation | `gateway-request-parity` | `docs/evidence/gateway-request-parity-v4.4.11.json`; `docs/GATEWAY_REQUEST_PREPARATION_PARITY.md`; `fixtures/gateway/request_preparation_cases.json`; `scripts/check_gateway_request_parity.py` |
| MCP protocol preparation | `mcp-protocol-parity` | `docs/evidence/mcp-protocol-parity-v4.4.11.json`; `docs/MCP_PROTOCOL_PREPARATION_PARITY.md`; `scripts/check_mcp_protocol_parity.py` |
| RAG parity | `rag-parity` | `docs/evidence/rag-parity-v4.4.11.json`; `docs/RAG_PARITY_BASELINE.md`; `scripts/check_rag_parity.py` |
| RAG document preparation | `rag-document-preparation-parity` | `docs/evidence/rag-document-preparation-parity-v4.4.11.json`; `docs/RAG_DOCUMENT_PREPARATION_PARITY.md`; `fixtures/rag/document_preparation_cases.json`; `scripts/check_rag_document_preparation_parity.py` |
| RAG vector binary transport | `rag-vector-binary-parity` | `docs/evidence/rag-vector-binary-parity-v4.4.11.json`; `scripts/check_rag_vector_binary_parity.py`; `docs/RAG_VECTOR_BINARY_TRANSPORT.md` |
| Rust coverage | `rust-coverage` | `docs/evidence/rust-coverage-v4.4.11.json`; `scripts/run_rust_coverage.py`; line coverage >= 80% |
| Rust sidecar performance | `rust-sidecar-performance` | `docs/evidence/rust-sidecar-performance-v4.4.11.json`; `scripts/run_rust_sidecar_benchmarks.py`; `docs/RUST_SIDECAR_PERFORMANCE.md` |

## Optional and frozen compatibility evidence

- Optional test-producer report: `docs/evidence/python-coverage-stability-v4.4.11.json`.
...
- Source context: `docs/evidence/evidence-source-context-v4.4.11.json`.
- Manifest: `docs/evidence/evidence-manifest-v4.4.11.json`.
- Detached manifest checksum: `docs/evidence/evidence-manifest-v4.4.11.json.sha256`.
- Final CI Artifact: `release-evidence-v4.4.11`.
- Release archive: `dist/deepseek-infra-4.4.11.zip`, its `.sha256`, `.manifest.json` and final preflight report.

No 4.4.11 Evidence is generated or committed from a dirty working tree. Formal PASS claims belong to the exact-merge CI assembly and its downloadable artifacts.
