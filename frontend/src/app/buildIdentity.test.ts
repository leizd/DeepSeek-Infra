...
describe("frontend build identity", () => {
  ...
  it("uses GITHUB_SHA for formal builds and marks local dirty builds", () => {
    ...
    expect(createFrontendBuildIdentity({ version: "4.3.5", sourceRevision: dirty }).buildId)
      .not.toBe(createFrontendBuildIdentity({ version: "4.3.5", sourceRevision: head }).buildId);
  });
});
