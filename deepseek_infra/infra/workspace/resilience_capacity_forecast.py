...
def _insufficient(target_id: str, horizon_days: int, *, now: datetime, sample_count: int, window_days: float) -> dict[str, Any]:
    payload = {
        "targetId": target_id,
...
def forecast_target_capacity(
    ...
    *,
    ...
    observations = resilience_capacity_history.list_capacity_observations(target_id)
    if len(observations) < MIN_SAMPLES:
        return _insufficient(target_id, horizon, now=current, sample_count=len(observations), window_days=0.0)
    ...
    if span_seconds < MIN_SPAN_SECONDS:
        return _insufficient(target_id, horizon, now=current, sample_count=len(observations), window_days=window_days)
    ...
    backtest = resilience_forecast_backtest.summarize_backtest(target_id)
