...
- **Coverage gate is 94.9% (4.6.1 combined branch floor after fail-closed index authority; restore 95.0 next)** (raised from 90% in v3.3.2), `source = ["deepseek_infra"]`, with branch measurement enabled. `--cov-fail-under=94.9` fails the run; branch coverage is reported but has no separate threshold yet. Lower locally with `pytest --no-cov` when iterating.
