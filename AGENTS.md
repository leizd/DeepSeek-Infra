...
- **Coverage gate is 90%** (raised from 85% in v3.3.1), `source = ["deepseek_infra"]`, with branch measurement enabled. `--cov-fail-under=90` fails the run; branch coverage is reported but has no separate threshold yet. Lower locally with `pytest --no-cov` when iterating.
