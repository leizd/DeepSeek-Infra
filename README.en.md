...
- Project-list, project-binding, skill-list, and memory-list mutations have explicit ownership scopes, keeping binding errors local.
- Recovery removes only failed mutations and preserves pending or paused work in the MutationCache and UI.
- Project and Skill lifecycle actions are mutually exclusive per entity; duplicate operations still share one request.
- Project uploads retain their original target after navigation and block conflicting rename/removal only for that project.
- Memory save is a first-class mutation, and clear is an exclusive barrier across save/remove work; expected conflicts remain business results.
...
See the [4.2.4 release notes](docs/releases/4.2.4.md), [frontend boundaries](docs/FRONTEND_MODULES.md), [upgrade guidance](docs/UPGRADING_TO_4_0.md), and [support policy](docs/4_0_SUPPORT_POLICY.md).
