...
def test_evidence_index_lists_edge_router_stabilization() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/edge-router-v{VERSION}.json" in index
...
def test_evidence_index_lists_context_taint() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/context-taint-v{VERSION}.json" in index
...
def test_evidence_index_lists_semantic_cache_benchmark() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/semantic-cache-onnx-v{VERSION}.json" in index
...
def test_evidence_index_lists_rag_document_preparation() -> None:
    ...
    assert f"docs/evidence/rust-sidecar-performance-v{VERSION}.json" in index
...
def test_evidence_index_lists_rag_vector_binary_transport() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/rag-vector-binary-parity-v{VERSION}.json" in index
...
def test_evidence_index_lists_workspace_core() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/workspace-v{VERSION}.json" in index
...
def test_evidence_index_lists_media_layer() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/media-v{VERSION}.json" in index
    assert f"evals/reports/media-v{VERSION}.json" in index
...
def test_evidence_index_lists_browser_control() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/browser-v{VERSION}.json" in index
    assert f"evals/reports/browser-v{VERSION}.json" in index
    assert "scripts/smoke_browser.py" in index
    assert "run_browser_eval.py" in index


def test_evidence_index_lists_frontend_browser_smoke() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/frontend-browser-v{VERSION}.json" in index
    assert "scripts/smoke_frontend_browser.py" in index
    assert "real Chromium" in index
    assert "Browser Control" in index


def test_evidence_index_lists_frontend_bundle_decomposition() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/frontend-bundle-v{VERSION}.json" in index
    assert "scripts/check_frontend_bundle.py" in index
...
def test_evidence_index_lists_automation_runtime() -> None:
    ...
    assert f"evals/reports/automation-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_system() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skills-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_workbench_ui() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skills-ui-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_builder() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-builder-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_packs() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-packs-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_eval_dashboard() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-eval-dashboard-v{VERSION}.json" in index
    assert f"evals/reports/skills-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_versioning() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-versioning-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_analytics() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-analytics-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_security() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-security-v{VERSION}.json" in index
...
def test_evidence_index_lists_skill_catalog() -> None:
    index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
    assert f"docs/evidence/skill-catalog-v{VERSION}.json" in index
