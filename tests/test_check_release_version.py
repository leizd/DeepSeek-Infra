...
def test_check_release_version_passes_against_the_real_repo() -> None:
    ...
    assert f"VERSION {CURRENT_VERSION}" in result.stdout


def test_fixture_tree_passes(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _write_repo(repo, FIXTURE_VERSION)
    result = _run("--root", str(repo))
    assert result.returncode == 0, result.stdout + result.stderr
    assert f"all release surfaces match VERSION {FIXTURE_VERSION}" in result.stdout
...
def test_ci_hardcoded_version_literal_fails(tmp_path: Path) -> None:
    ...
    ci.write_text(ci.read_text(encoding="utf-8") + f"- run: python scripts/release.py --version {FIXTURE_VERSION}\n", encoding="utf-8")
    result = _run("--root", str(repo))
    assert result.returncode == 1
...
def test_strict_branch_accepts_matching_and_plain_branches(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _write_repo(repo, FIXTURE_VERSION)
    matching = _run("--root", str(repo), "--strict-branch", env={"GITHUB_HEAD_REF": f"codex/release-{FIXTURE_VERSION}-durable-checkpoints"})
...
def test_require_release_note(tmp_path: Path) -> None:
    ...
    notes.mkdir()
    (notes / f"{FIXTURE_VERSION}.md").write_text(f"# DeepSeek Infra {FIXTURE_VERSION} — Test Release\n", encoding="utf-8")
