...
def test_build_manifest_has_required_fields(tmp_path: Path) -> None:
    ...
    expected_evidence = (
        *evidence_paths("4.4.2"),
        "docs/evidence/evidence-source-context-v4.4.2.json",
        "docs/evidence/evidence-manifest-v4.4.2.json",
        "docs/evidence/evidence-manifest-v4.4.2.json.sha256",
    )
    assert tuple(manifest["evidence"]) == expected_evidence
    assert manifest["evidenceManifest"] == {}
    assert manifest["gaEvidence"] == "docs/evidence/ga-v4.4.2.json"
