...
def test_build_manifest_has_required_fields(tmp_path: Path) -> None:
    ...
    manifest = release_manifest.build_manifest(
        ...
        rust_sidecar_image_tag="deepseek-rust-gateway:4.2.0",
    ...
    assert manifest["rustSidecarImage"] == {
        "tag": "deepseek-rust-gateway:4.2.0",
        "digest": "sha256:1234",
    }
    ...
    assert manifest["gaEvidence"] == "docs/evidence/ga-v4.2.0.json"
    ...
    assert "docs/evidence/edge-router-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/ga-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/workspace-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/context-taint-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/semantic-cache-onnx-v4.2.0.json" in manifest["evidence"]
    assert "docs/RUST_CANDIDATE_AUDIT_3_4.md" in manifest["evidence"]
    assert "docs/evidence/media-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/browser-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/frontend-browser-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/frontend-bundle-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/automation-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skills-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skills-ui-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-builder-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-packs-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-eval-dashboard-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-versioning-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-analytics-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-security-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-catalog-v4.2.0.json" in manifest["evidence"]
    assert "evals/reports/skills-v4.2.0.json" in manifest["evidence"]
    assert "evals/reports/media-v4.2.0.json" in manifest["evidence"]
    assert "evals/reports/browser-v4.2.0.json" in manifest["evidence"]
    assert "evals/reports/automation-v4.2.0.json" in manifest["evidence"]
    ...
    assert "docs/evidence/rust-sidecar-performance-v4.2.0.json" in manifest["evidence"]
    assert "docs/evidence/rag-vector-binary-parity-v4.2.0.json" in manifest["evidence"]
    ...
    assert "docs/releases/4.2.0.md" in manifest["evidence"]
