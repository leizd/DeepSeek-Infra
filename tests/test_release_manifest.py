...
def test_build_manifest_has_required_fields(tmp_path: Path) -> None:
    ...
    assert manifest["gaEvidence"] == "docs/evidence/ga-v3.5.0.json"
    ...
    assert "docs/evidence/edge-router-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/ga-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/workspace-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/context-taint-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/semantic-cache-onnx-v3.5.0.json" in manifest["evidence"]
    assert "docs/RUST_CANDIDATE_AUDIT_3_4.md" in manifest["evidence"]
    assert "docs/evidence/media-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/browser-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/automation-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skills-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skills-ui-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-builder-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-packs-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-eval-dashboard-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-versioning-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-analytics-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-security-v3.5.0.json" in manifest["evidence"]
    assert "docs/evidence/skill-catalog-v3.5.0.json" in manifest["evidence"]
    assert "evals/reports/skills-v3.5.0.json" in manifest["evidence"]
    assert "evals/reports/media-v3.5.0.json" in manifest["evidence"]
    assert "evals/reports/browser-v3.5.0.json" in manifest["evidence"]
    assert "evals/reports/automation-v3.5.0.json" in manifest["evidence"]
