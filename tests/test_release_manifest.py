...
def test_build_manifest_has_required_fields(tmp_path: Path) -> None:
    ...
    manifest = release_manifest.build_manifest(
        ...
        rust_sidecar_image_tag="deepseek-rust-gateway:4.3.6",
    ...
    assert manifest["rustSidecarImage"] == {
        "tag": "deepseek-rust-gateway:4.3.6",
        "digest": "sha256:1234",
    }
    assert manifest["runtimeDefaults"]["authoritativeRuntime"] == "python"
    assert manifest["runtimeDefaults"]["defaultCompose"] == "python-only"
    expected_evidence = (
        *evidence_paths("4.3.6"),
        "docs/evidence/evidence-source-context-v4.3.6.json",
        "docs/evidence/evidence-manifest-v4.3.6.json",
        "docs/evidence/evidence-manifest-v4.3.6.json.sha256",
    )
    assert tuple(manifest["evidence"]) == expected_evidence
    assert manifest["evidenceManifest"] == {}
    assert manifest["gaEvidence"] == "docs/evidence/ga-v4.3.6.json"
