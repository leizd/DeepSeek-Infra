...
class EncodingRegressionTests(unittest.TestCase):
    ...
    def test_release_version_sync_current(self) -> None:
        ...
        self.assertIn("version-4.3.0-blue", readme)
        self.assertIn('app_version: str = "4.3.0"', config)
        self.assertIn("deepseek-infra:4.3.0", dockerfile)
        self.assertIn('org.opencontainers.image.version="4.3.0"', dockerfile)
        self.assertIn('versionName "4.3.0"', build_gradle)
        self.assertIn("versionCode 400024", build_gradle)
        self.assertIn('<meta name="deepseek-infra-version" content="4.3.0" />', frontend)
        self.assertIn("## [4.3.0] - Workspace Demand Loading", changelog)
        self.assertIn("Personal AI Runtime GA", readme)
        self.assertIn("python scripts/generate_release_evidence.py --version 4.3.0", ci)
        self.assertIn("evidence-context:", ci)
        self.assertIn("evidence-assembly:", ci)
        self.assertIn("release-package:", ci)
        self.assertIn("name: release-evidence-v4.3.0", ci)
        self.assertNotIn("scripts/release.py --version 4.3.0 --dry-run", ci)
    ...
    def test_release_docs_are_registered(self) -> None:
        ...
        for rel in (
            ...
            "docs/releases/4.3.0.md",
        ):
            self.assertTrue((ROOT / rel).is_file(), rel)
            self.assertIn(rel, readme)

        self.assertIn("docs/evidence/ga-v4.3.0.json", evidence_index)
    ...
    def test_required_evidence_json_is_current_and_passes(self) -> None:
        ...
        self.assertIn("docs/evidence/ga-v4.3.0.json", required)
        self.assertIn("docs/evidence/frontend-browser-v4.3.0.json", required)
        for rel in required:
