...
class EncodingRegressionTests(unittest.TestCase):
    ...
    def test_v218_verifiable_readme_and_engineering_assets_are_present(self) -> None:
        """v2.1.7：从「宣传型」README 走向「可验证型」+ 依赖锁定 / 体检 / 安全模式 / CI 门禁。"""
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        sw = Path("static/sw.js").read_text(encoding="utf-8")

        # 1. README 可验证型第一屏：30 秒启动 + 截图，宏大叙事下沉 ARCHITECTURE
        self.assertIn("## 30 秒启动", readme)
        self.assertIn("## 截图", readme)
        self.assertIn("docs/assets/screenshots/01-chat.png", readme)
        # 真实截图入库（4 张）
        for shot in ("01-chat.png", "02-rag-citation.png", "03-agent-dag.png", "04-trace.png"):
            with self.subTest(shot=shot):
                self.assertTrue(Path("docs/assets/screenshots", shot).is_file())

        # 2. 一键体检
        doctor = Path("scripts/doctor.py").read_text(encoding="utf-8")
        self.assertIn("DeepSeek Infra Doctor", doctor)
        self.assertTrue(Path("tests/test_doctor.py").is_file())

        # 3. 安全模式开关
        self.assertIn("security_mode", config)
        self.assertIn("strict_security", config)
        self.assertIn("SECURITY_MODE", Path(".env.example").read_text(encoding="utf-8"))
        self.assertIn("SECURITY_MODE=strict", Path("docs/SECURITY.md").read_text(encoding="utf-8"))

        # 4. 依赖规范化 + 锁定
        pyproject = Path("pyproject.toml").read_text(encoding="utf-8")
        self.assertIn("[project]", pyproject)
        self.assertIn('name = "deepseek-infra"', pyproject)
        self.assertIn('version = "2.1.7"', pyproject)
        self.assertTrue(Path("requirements.lock").is_file())

        # 5. Docker 多阶段构建 + 多架构文档
        dockerfile = Path("Dockerfile").read_text(encoding="utf-8")
        self.assertIn("AS builder", dockerfile)
        self.assertIn("/opt/venv", dockerfile)
        self.assertIn("buildx", Path("docs/DEPLOYMENT.md").read_text(encoding="utf-8"))

        # 6. RAG 两档 embedding 文档化
        self.assertIn("LOCAL_RAG_EMBEDDING_PROVIDER", readme)
        self.assertIn("requirements-rag.txt", readme)

        # 7. CI 提门槛 + 评测进门禁
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        self.assertIn("--cov-fail-under=75", ci)
        self.assertIn("run_tool_eval.py", ci)
        self.assertIn("--fail-under-recall", ci)
        self.assertIn("--fail-under-recall", Path("evals/runners/run_rag_eval.py").read_text(encoding="utf-8"))
