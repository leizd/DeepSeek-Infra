...
def test_capture_source_context_rejects_dirty_tree(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.delenv("GITHUB_SHA", raising=False)
    try:
        revision_module.capture_source_context(tmp_path, "4.3.3", generator="test")
...
def test_capture_schema_v2_context_binds_github_identity(tmp_path: Path, monkeypatch) -> None:
    ...
    context = revision_module.capture_source_context(tmp_path, "4.3.3", generator="test")
    ...
    assert revision_module.validate_source_context(context, version="4.3.3", expected_revision=revision) == []
