...
> **4.8.0 — 签名联邦与跨 Fleet 灾难恢复（候选实现完成）**。Gate A 已关闭不可变 Wave Schedule identity、可续租 runner lease 与真实进程接管；Signed Federation 已实现 dedicated Fleet signing identity、operator-pinned peer trust、Receiver-controlled ingress、签名异地副本证明和 production federated DR，两个 Fleet 始终保持独立 Authority。详见 [发布说明](docs/releases/4.8.0.md)、[运维手册](docs/runbooks/SIGNED_FEDERATION_DR.md) 与 [Evidence 索引](docs/EVIDENCE_INDEX.md)。

当前稳定版本说明：[4.7.6](docs/releases/4.7.6.md)。4.8.0 的本地真实 MinIO 候选验证已完成；正式 release-ready 仍等待最终 PR head/merge SHA 的全量 CI、三个 exact federation artifacts 与 Evidence Assembly。
