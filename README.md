...
![版本](https://img.shields.io/badge/version-3.9.0-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-95%25-brightgreen)
![许可证](https://img.shields.io/badge/license-MIT-black)

> **3.9.0 stable line:** the existing Rust RAG vector-ranking delegate now has an explicit compact little-endian `f64` transport alongside its compatible JSON endpoint. Binary remains opt-in and JSON remains the transport default; Python still performs the complete authoritative ranking and directly falls back on timeout, failure, malformed response, or parity divergence. No Rust delegate or ownership boundary was added: all Rust delegates and sidecar deployment remain disabled by default, default Compose remains Python-only, and persistence remains Python-owned. See [the binary contract](docs/RAG_VECTOR_BINARY_TRANSPORT.md), [the performance evidence](docs/RUST_SIDECAR_PERFORMANCE.md), and [the hybrid runbook](docs/RUST_HYBRID_RUNTIME_RUNBOOK.md).
...
**先验证，再相信**：[实现状态矩阵](docs/IMPLEMENTATION_STATUS.md)（13 个模块各自的代码 / 测试 / Demo 落地程度）· [2 分钟 Demo](docs/DEMO.md) · [Benchmarks](#benchmarks基准与评测) · [部署](docs/DEPLOYMENT.md) · [威胁模型](docs/THREAT_MODEL.md)
...
| 11 | **Multimodal Media Layer** | [`infra/media/`](deepseek_infra/infra/media/) | Image / PDF / 音频 / 视频 / 网页 snapshot 的导入、分段、citation、project export 与 media-to-RAG 生命周期。 |
| 12 | **Browser Control Runtime** | [`infra/browser/`](deepseek_infra/infra/browser/) | 受控浏览器会话、安全门控动作、私有地址拦截、Browser-to-Media snapshot、网页内容 RAG 索引与审计日志。 |
| 13 | **Automation Runtime** | [`infra/automation/`](deepseek_infra/infra/automation/) | 本地自动化定义、trigger/condition/action/policy、run history、trace linkage 与 release-gated smoke/eval evidence。 |
...
- `POST /api/edge/route-preview` 可在不加载模型的情况下解释本轮会走端侧还是云端，并返回 `reason` 与当前 Edge 状态。
...
**Release preflight (v3.9.0)**: Run `python scripts/doctor.py --offline`, `python scripts/smoke_ga.py --offline --out docs/evidence/ga-v3.9.0.json`, `python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v3.9.0.json`, `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v3.9.0.json`, `python scripts/smoke_media.py --offline --out docs/evidence/media-v3.9.0.json`, `python scripts/smoke_browser.py --offline --out docs/evidence/browser-v3.9.0.json --version 3.9.0`, `python scripts/smoke_automation.py --offline --out docs/evidence/automation-v3.9.0.json --version 3.9.0`, `python scripts/smoke_skills.py --offline --out docs/evidence/skills-v3.9.0.json`, `python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v3.9.0.json`, `python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v3.9.0.json`, `python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v3.9.0.json`, `python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v3.9.0.json --report-out evals/reports/skills-v3.9.0.json`, `python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v3.9.0.json`, `python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v3.9.0.json`, `python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v3.9.0.json`, `python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v3.9.0.json`, `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v3.9.0.json`, and `python evals/runners/run_media_eval.py --strict --out evals/reports/media-v3.9.0.json`, and `python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v3.9.0.json --version 3.9.0`, and `python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v3.9.0.json --version 3.9.0`; then run `python scripts/check_mcp_protocol_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/mcp-protocol-parity-report.json`, `python scripts/check_rag_document_preparation_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/rag-document-preparation-parity-report.json`, `python scripts/check_rag_vector_binary_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/rag-vector-binary-parity.json`, `python scripts/preflight_release.py --version 3.9.0 --ga`, and `python scripts/smoke_release.py --offline`. Release manifest quality gates include `gatewayRequestParity`, `mcpProtocolParity`, `ragDocumentPreparationParity`, `ragVectorBinaryParity`, `rustSidecarPerformance`, `edgeRouter`, `workspaceCore`, `mediaLayer`, `browserControl`, `automationRuntime`, `contextTaint`, `skillSystem`, `skillWorkbench`, `skillBuilder`, `skillPacks`, `skillEvalDashboard`, `skillVersioning`, `skillAnalytics`, `skillSecurity`, and `skillCatalog`.
...
- [docs/IMPLEMENTATION_STATUS.md](docs/IMPLEMENTATION_STATUS.md) — **实现状态矩阵**：13 个模块的代码 / 测试 / Demo 落地程度与缺口。
