...
![版本](https://img.shields.io/badge/version-3.10.0-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-95%25-brightgreen)
![许可证](https://img.shields.io/badge/license-MIT-black)

> **3.10.0 stable line:** semantic-cache rows now retain the existing six-decimal JSON embedding and dual-write the same normalized values as `f64le-v1` SQLite BLOBs. Explicit binary Rust ranking can assemble one unchanged `DSVRNK01` request directly from valid BLOBs while mixed/legacy/corrupt rows fall back per row to JSON and the complete Python parity scan remains authoritative. Startup only adds columns; the optional migration is explicit, batched, dry-run by default, and resumable. Rust still never reads SQLite, all Rust flags and sidecar deployment remain disabled by default, the transport default remains JSON, and default Compose remains Python-only. See [the storage/migration contract](docs/SEMANTIC_CACHE_BINARY_EMBEDDINGS.md), [the binary contract](docs/RAG_VECTOR_BINARY_TRANSPORT.md), [the performance evidence](docs/RUST_SIDECAR_PERFORMANCE.md), and [the hybrid runbook](docs/RUST_HYBRID_RUNTIME_RUNBOOK.md).
...
**Release preflight (v3.10.0)**: Run `python scripts/doctor.py --offline`, `python scripts/smoke_ga.py --offline --out docs/evidence/ga-v3.10.0.json`, `python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v3.10.0.json`, `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v3.10.0.json`, `python scripts/smoke_media.py --offline --out docs/evidence/media-v3.10.0.json`, `python scripts/smoke_browser.py --offline --out docs/evidence/browser-v3.10.0.json --version 3.10.0`, `python scripts/smoke_automation.py --offline --out docs/evidence/automation-v3.10.0.json --version 3.10.0`, `python scripts/smoke_skills.py --offline --out docs/evidence/skills-v3.10.0.json`, `python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v3.10.0.json`, `python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v3.10.0.json`, `python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v3.10.0.json`, `python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v3.10.0.json --report-out evals/reports/skills-v3.10.0.json`, `python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v3.10.0.json`, `python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v3.10.0.json`, `python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v3.10.0.json`, `python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v3.10.0.json`, `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v3.10.0.json`, and `python evals/runners/run_media_eval.py --strict --out evals/reports/media-v3.10.0.json`, and `python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v3.10.0.json --version 3.10.0`, and `python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v3.10.0.json --version 3.10.0`; then run `python scripts/check_mcp_protocol_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/mcp-protocol-parity-report.json`, `python scripts/check_rag_document_preparation_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/rag-document-preparation-parity-report.json`, `python scripts/check_rag_vector_binary_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/rag-vector-binary-parity.json`, `python scripts/preflight_release.py --version 3.10.0 --ga`, and `python scripts/smoke_release.py --offline`. Release manifest quality gates include `gatewayRequestParity`, `mcpProtocolParity`, `ragDocumentPreparationParity`, `ragVectorBinaryParity`, `semanticCacheBinaryEmbeddings`, `rustSidecarPerformance`, `edgeRouter`, `workspaceCore`, `mediaLayer`, `browserControl`, `automationRuntime`, `contextTaint`, `skillSystem`, `skillWorkbench`, `skillBuilder`, `skillPacks`, `skillEvalDashboard`, `skillVersioning`, `skillAnalytics`, `skillSecurity`, and `skillCatalog`.
