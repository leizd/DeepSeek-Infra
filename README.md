...
![版本](https://img.shields.io/badge/version-3.8.0-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-95%25-brightgreen)
![许可证](https://img.shields.io/badge/license-MIT-black)

> **3.8.0 stable line:** the existing Rust sidecar now has a locked release-mode, layered performance baseline, persistent Python-side HTTP connections, bounded Prometheus metrics, safe correlation tracing, and per-delegate timing diagnostics. No Rust delegate or ownership boundary was added: Python remains authoritative, all Rust delegates and the sidecar deployment remain disabled by default, defensive parity validation and fallback remain mandatory, default Compose remains Python-only, and persistence remains Python-owned. See [the performance and observability report](docs/RUST_SIDECAR_PERFORMANCE.md), [the hybrid runbook](docs/RUST_HYBRID_RUNTIME_RUNBOOK.md), and [the historical RC notes](docs/releases/4.0.0-rc.1.md).
...
3.8.0 adds no delegate. It makes every existing delegate measurable in a locked release build, reuses bounded process-local HTTP connections, separates Python preparation/serialization/transport/Rust processing/Python validation/total time, and exposes low-cardinality sidecar metrics on the existing listener. The benchmark keeps cold start separate from warmed HTTP and full integration, reports slower Rust cases, and treats public-runner latency as informational. See [RUST_SIDECAR_PERFORMANCE.md](docs/RUST_SIDECAR_PERFORMANCE.md).
...
**Release preflight (v3.8.0)**: Run `python scripts/doctor.py --offline`, `python scripts/smoke_ga.py --offline --out docs/evidence/ga-v3.8.0.json`, `python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v3.8.0.json`, `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v3.8.0.json`, `python scripts/smoke_media.py --offline --out docs/evidence/media-v3.8.0.json`, `python scripts/smoke_browser.py --offline --out docs/evidence/browser-v3.8.0.json --version 3.8.0`, `python scripts/smoke_automation.py --offline --out docs/evidence/automation-v3.8.0.json --version 3.8.0`, `python scripts/smoke_skills.py --offline --out docs/evidence/skills-v3.8.0.json`, `python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v3.8.0.json`, `python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v3.8.0.json`, `python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v3.8.0.json`, `python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v3.8.0.json --report-out evals/reports/skills-v3.8.0.json`, `python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v3.8.0.json`, `python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v3.8.0.json`, `python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v3.8.0.json`, `python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v3.8.0.json`, `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v3.8.0.json`, and `python evals/runners/run_media_eval.py --strict --out evals/reports/media-v3.8.0.json`, and `python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v3.8.0.json --version 3.8.0`, and `python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v3.8.0.json --version 3.8.0`; then run `python scripts/check_mcp_protocol_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/mcp-protocol-parity-report.json`, `python scripts/check_rag_document_preparation_parity.py --base-url http://127.0.0.1:8787 --strict --report artifacts/rag-document-preparation-parity-report.json`, `python scripts/preflight_release.py --version 3.8.0 --ga`, and `python scripts/smoke_release.py --offline`. Release manifest quality gates include `gatewayRequestParity`, `mcpProtocolParity`, `ragDocumentPreparationParity`, `rustSidecarPerformance`, `edgeRouter`, `workspaceCore`, `mediaLayer`, `browserControl`, `automationRuntime`, `contextTaint`, `skillSystem`, `skillWorkbench`, `skillBuilder`, `skillPacks`, `skillEvalDashboard`, `skillVersioning`, `skillAnalytics`, `skillSecurity`, and `skillCatalog`.
