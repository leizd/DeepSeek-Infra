...
![版本](https://img.shields.io/badge/version-2.2.6-blue)
...
与之配套的**质量评测**在 [evals/](evals/)（全部离线可跑）：RAG Recall@5 1.000 / Citation Accuracy 1.000、Agent 工具调用与完成率打分、26 个固定攻防用例的 **Tool Policy Pass Rate 1.000 / Prompt Injection Defense Pass 1.000**（`python evals/runners/run_tool_eval.py`），以及对抗注入小语料的 `block_rate` / `false_positive_rate` / `bypass_rate` 与 **soft gate**（`python evals/runners/run_injection_adversarial.py`），详见 [evals/README.md](evals/README.md)。本地安全能力复现最小命令集见 [docs/SECURITY_SMOKE.md](docs/SECURITY_SMOKE.md)。
