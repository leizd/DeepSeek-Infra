...
![版本](https://img.shields.io/badge/version-3.5.0-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-95%25-brightgreen)
![许可证](https://img.shields.io/badge/license-MIT-black)

> **3.5.0 stable line:** optional Rust Gateway request preparation validates and normalizes credential-free, non-streaming upstream bodies through `POST /gateway/request/prepare`. Python still owns credentials, provider routing, upstream HTTP, streaming, retries, tools, cache policy, and tracing lifecycles. The delegate is off by default, malformed or unavailable sidecars fall back to Python, and default Compose remains Python-only. The published `v4.0.0-rc.1` remains an architecture preview and release-flow rehearsal; it does not move the stable line to 4.0. See [the parity contract](docs/GATEWAY_REQUEST_PREPARATION_PARITY.md), [the hybrid runbook](docs/RUST_HYBRID_RUNTIME_RUNBOOK.md), and [the historical RC notes](docs/releases/4.0.0-rc.1.md).
...
**Release preflight (v3.5.0)**: Run `python scripts/doctor.py --offline`, `python scripts/smoke_ga.py --offline --out docs/evidence/ga-v3.5.0.json`, `python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v3.5.0.json`, `python scripts/smoke_edge_router.py --offline --out docs/evidence/edge-router-v3.5.0.json`, `python scripts/smoke_media.py --offline --out docs/evidence/media-v3.5.0.json`, `python scripts/smoke_browser.py --offline --out docs/evidence/browser-v3.5.0.json --version 3.5.0`, `python scripts/smoke_automation.py --offline --out docs/evidence/automation-v3.5.0.json --version 3.5.0`, `python scripts/smoke_skills.py --offline --out docs/evidence/skills-v3.5.0.json`, `python scripts/smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v3.5.0.json`, `python scripts/smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v3.5.0.json`, `python scripts/smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v3.5.0.json`, `python scripts/smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v3.5.0.json --report-out evals/reports/skills-v3.5.0.json`, `python scripts/smoke_skill_versioning.py --offline --out docs/evidence/skill-versioning-v3.5.0.json`, `python scripts/smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v3.5.0.json`, `python scripts/smoke_skill_security.py --offline --out docs/evidence/skill-security-v3.5.0.json`, `python scripts/smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v3.5.0.json`, `python scripts/smoke_context_taint.py --offline --out docs/evidence/context-taint-v3.5.0.json`, and `python evals/runners/run_media_eval.py --strict --out evals/reports/media-v3.5.0.json`, and `python evals/runners/run_browser_eval.py --strict --out evals/reports/browser-v3.5.0.json --version 3.5.0`, and `python evals/runners/run_automation_eval.py --strict --out evals/reports/automation-v3.5.0.json --version 3.5.0`; then run `python scripts/preflight_release.py --version 3.5.0 --ga` and `python scripts/smoke_release.py --offline`. Release manifest quality gates include `edgeRouter`, `workspaceCore`, `mediaLayer`, `browserControl`, `automationRuntime`, `contextTaint`, `skillSystem`, `skillWorkbench`, `skillBuilder`, `skillPacks`, `skillEvalDashboard`, `skillVersioning`, `skillAnalytics`, `skillSecurity`, and `skillCatalog`.
