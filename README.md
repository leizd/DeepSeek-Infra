...
![版本](https://img.shields.io/badge/version-4.3.6-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-95%25-brightgreen)
![许可证](https://img.shields.io/badge/license-MIT-black)

> **4.3.6 Cross-Tab Checkpoint Arbitration & Bounded Storage：** 对话持久化从整库 V2 快照分片为按会话的 V3 检查点（`deepseek-infra.session.v3.head.<cid>` + 摘要校验的 `.snapshot.<cid>.<revision>`，revision `<seq>.<tabId>` 带 parentRevision 链），编辑单个会话只序列化该会话；保留 current+previous+未解决冲突、每次提交最多删除 2 个分片并配预算化空闲孤儿 GC，清理保持 O(1)，V2 journal 在首个验证通过的 V3 提交后迁移。提交在排他 Web Lock（`deepseek-conversation-checkpoint`，含等价无锁 sibling 检测回退）内进行，临界区内重读共享 head，过期标签页无法静默覆盖，其分支保留为会话级冲突副本（`session.v3.conflict.<cid>`），经通知栏「查看最新 / 保留副本」恢复为独立「（冲突副本）」会话；`deepseek-conversation-sync` BroadcastChannel 仅使远端干净副本失效，绝不切换当前查看会话。删除先提交 tombstone（`session.v3.tombstone.<cid>`）再动 UI，过期标签页无法复活 id，其编辑仅以确定性「（恢复副本）」幸存；tombstone 有界（30 天 / 50 条），仅在所有存活标签页 lease（`session.v3.tab.<tabId>`，5 分钟过期）都看到删除后 GC。当前会话选择下沉为每标签页 sessionStorage 状态，切换不再触发共享提交。保存调度：编辑 300ms 合并、流式 1Hz 限频带尾部提交、done/error/interrupted、删除、page-hide 与构建更新激活时立即提交。配额耗尽确定性降级：先剥离可重建的 `data:` 图片预览（≥1KB，保留名称/类型/大小/fileId/文本），再截断超大 timeline 原始负载（>8KB），并在检查点记录 compaction `{level, removedPreviewBytes, reason:"storage-pressure"}`；用户正文永不被触碰，失败保留旧 head 并给出导出/清理指引。页面退出同步写入每标签页 Recovery Capsule（`session.v3.recovery.<tabId>`），启动时在写锁内恰好对账一次（干净⇒原地提交，冲突/已删除⇒确定性 `<cid>.recovered.<tabId>` 副本），崩溃标签页的 capsule 在 lease 死亡后回收。构建侧：vendor 运行时（react/react-dom/react-router/tanstack）经 Vite `manualChunks` 拆分为独立可缓存 core chunk，入口产物 390KB→157KB，vendor hash 在纯应用发布间保持稳定，Bundle 预算不变。参见 [4.3.6 发布说明](docs/releases/4.3.6.md)（上一版 [4.3.5](docs/releases/4.3.5.md)，再上一版 [4.3.4](docs/releases/4.3.4.md)）、[Evidence 索引](docs/EVIDENCE_INDEX.md)、[前端边界](docs/FRONTEND_MODULES.md)和[支持策略](docs/4_0_SUPPORT_POLICY.md)。

**4.3.6 validation:** 前端单元与合同测试（68 文件 / 567 测试）覆盖分片提交只序列化单个会话、O(1) 清理、锁内重读 head 防过期写入覆盖、冲突副本恢复、tombstone 防复活、每标签页选择独立、1Hz 流式背压、配额两级降级与 compaction 元数据、Capsule 恰好一次恢复；真实 Chromium smoke 新增 10 个跨标签页检查键（crossTabDisjointWritesPreserved、sameConversationConflictDetected、staleWriterCannotAdvanceHead、deletedConversationNotResurrected、tabSelectionRemainsIndependent、checkpointCleanupRemainsConstantTime、streamCheckpointRateBounded、storagePressureCompactionLossless、recoveryCapsuleReconciledOnce、conflictBranchRecoverable）并由 preflight `>= (4,3,6)` 版本检查门禁，本地两轮 120/120 通过；check_release_version.py 版本一致性门禁先行。4.3.5 持久化检查点、4.3.4 重载事务、4.3.3 发现/静默重载、4.3.2 构建身份/Lease、现有 Bundle 预算与 4.2.8 exact-merge Evidence 装配链保持不变。
...
**架构图语言 / Diagram language：**点击下方标题选择中文或 English。

<details open>
<summary><strong>中文架构图</strong></summary>

![DeepSeek Infra 中文架构总览](docs/assets/architecture.zh-CN.svg)
...
| Chat · 搜索来源与引用 | Agent Run · Timeline 与 Activity |
| --- | --- |
| ![Chat with search sources and citations](docs/assets/react-chat-overview.png) | ![Agent run timeline and activity](docs/assets/react-agent-run.png) |

| Trace 瀑布图 | 文件阅读器 |
| --- | --- |
| ![Trace waterfall](docs/assets/react-trace-waterfall.png) | ![File reader](docs/assets/react-file-reader.png) |

| 诊断面板 | Activity 侧栏 |
| --- | --- |
| ![Diagnostics panel](docs/assets/react-diagnostics.png) | ![Activity drawer](docs/assets/react-activity-drawer.png) |

| Projects 工作台 | Skills 面板 |
| --- | --- |
| ![Projects workspace](docs/assets/react-projects.png) | ![Skills panel](docs/assets/react-skills.png) |

| Memory 面板 | |
| --- | --- |
| ![Memory panel](docs/assets/react-memory.png) | |

> 截图反映当前 DeepSeek Infra React UI。请参见 [2 分钟 Demo](docs/DEMO.md) 了解各功能的实际演示。
...
Rust Gateway sidecar（3.0.3 起）已迁移为独立文档：RAG 向量排序（JSON 与 `f64le` 二进制传输）、Gateway / MCP / RAG 请求准备委托、语义缓存二进制嵌入、Rust Policy、sidecar 性能基准与 Docker Compose 启动方式，详见 [docs/RUST_GATEWAY_SIDECAR.md](docs/RUST_GATEWAY_SIDECAR.md)。
...
所有人工维护的 Markdown 文档顶部都提供“中文 / English”入口；自动生成的证据、评测报告和 `AGENTS.md` 保持原样，避免破坏可复现证据或代理指令。
